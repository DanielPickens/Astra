...
save_logs() {
    ...
    cat <<EOF | astra-robot -key-from-env-var ROBOT_KEY -pr-comment ${GIT_PR_NUMBER} -pipeline "${NAME}"
...
# with values: "team: astra" and "type: testing"
cleanup_namespaces() {
    ...
    for PROJECT in ${PROJECTS}; do
        ...
        if [[ "${TYPE}" -eq "testing" ]] && [[ "${TEAM}" -eq "astra" ]]; then
