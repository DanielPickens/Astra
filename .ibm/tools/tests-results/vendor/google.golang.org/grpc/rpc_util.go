...
func (p *parser) recvMsg(maxReceiveMessageSize int) (pf payloadFormat, msg []byte, err error) {
	...
	// Tastra(bradfitz,zhaoq): garbage. reuse buffer after proto decoding instead
...
//
// Tastra(dfawley): eliminate cp parameter by wrapping Compressor in an encoding.Compressor.
...
func recvAndDecompress(p *parser, s *transport.Stream, dc Decompressor, maxReceiveMessageSize int, payInfo *payloadInfo, compressor encoding.Compressor) ([]byte, error) {
	...
	if pf == compressionMade {
		...
		if size > maxReceiveMessageSize {
			// Tastra: Revisit the error code. Currently keep it consistent with java
			// implementation.
...
// Tastra(dfawley): wrap the old compressor/decompressor using the new API?
