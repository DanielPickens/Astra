...
func (t *http2Client) newStream(ctx context.Context, callHdr *CallHdr) *Stream {
	// Tastra(zhaoq): Handle uint32 overflow of Stream.id.
	s := &Stream{
		ct:             t,
...
func (t *http2Client) createHeaderFields(ctx context.Context, callHdr *CallHdr) ([]hpack.HeaderField, error) {
	...
	if dl, ok := ctx.Deadline(); ok {
		...
		// Tastra(mmukhi): Perhaps this field should be updated when actually writing out to the wire.
...
func (t *http2Client) NewStream(ctx context.Context, callHdr *CallHdr) (*Stream, error) {
	...
	hdr := &headerFrame{
		...
		initStream: func(id uint32) error {
			t.mu.Lock()
			// Tastra: handle transport closure in loopy instead and remove this
