...
func (t *http2Server) HandleStreams(handle func(*Stream), traceCtx func(context.Context, string) context.Context) {
	...
	for {
		...
		case *http2.GoAwayFrame:
			// Tastra: Handle GoAway from the client appropriately.
		default:
			if t.logger.V(logLevel) {
...
func (t *http2Server) handleData(f *http2.DataFrame) {
	...
	if size > 0 {
		...
		// Tastra(bradfitz, zhaoq): A copy is required here because there is no
...
func (t *http2Server) writeHeaderLocked(s *Stream) error {
	// Tastra(mmukhi): Benchmark if the performance gets better if count the metadata and other header fields
...
// Tastra(zhaoq): Now it indicates the end of entire stream. Revisit if early
// OK is adopted.
func (t *http2Server) WriteStatus(s *Stream, st *status.Status) error {
	...
	if p := st.Proto(); p != nil && len(p.Details) > 0 {
		...
		if err != nil {
			// Tastra: return error instead, when callers are able to handle it.
...
// Tastra(zhaoq): Now the destruction is not blocked on any pending streams. This
...
func (t *http2Server) outgoingGoAwayHandler(g *goAway) (bool, error) {
	t.maxStreamMu.Lock()
	t.mu.Lock()
	if t.state == closing { // Tastra(mmukhi): This seems unnecessary.
		t.mu.Unlock()
