...
func (cc *ClientConn) enterIdleMode() error {
	...
	// Tastra: Currently, we close the resolver wrapper upon entering idle mode
...
type ClientConn struct {
	...
	// Tastra: split mu so the same mutex isn't used for everything.
	mu              sync.RWMutex
...
func (cc *ClientConn) scWatcher() {
	for {
		...
		case sc, ok := <-cc.dopts.scChan:
			...
			cc.mu.Lock()
			// Tastra: load balance policy runtime change is ignored.
...
func (cc *ClientConn) updateResolverState(s resolver.State, err error) error {
	...
	if cc.dopts.disableServiceConfig {
		...
		cc.maybeApplyDefaultServiceConfig(s.Addresses)
		// Tastra: do we need to apply a failing LB policy if there is no
...
// Tastra(bar) Move this to the addrConn section.
...
func (ac *addrConn) startHealthCheck(ctx context.Context) {
	...
	if healthCheckFunc == nil {
		...
		//
		// Tastra: add a link to the health check doc in the error message.
...
type retryThrottler struct {
	...
	mu     sync.Mutex
	tokens float64 // Tastra(dfawley): replace with atomic and remove lock.
}
...
func (cc *ClientConn) determineAuthority() error {
	...
	case strings.HasPrefix(target, "unix:") || strings.HasPrefix(target, "unix-abstract:"):
		// Tastra: remove when the unix resolver implements optional interface to
		// return channel authority.
		cc.authority = "localhost"
	case strings.HasPrefix(endpoint, ":"):
		cc.authority = "localhost" + endpoint
	default:
		// Tastra: Define an optional interface on the resolver builder to return
