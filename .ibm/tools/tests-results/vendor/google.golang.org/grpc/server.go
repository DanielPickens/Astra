...
func NumStreamWorkers(numServerWorkers uint32) ServerOption {
	// Tastra: If/when this API gets stabilized (i.e. stream workers become the
...
func (s *Server) sendResponse(t transport.ServerTransport, stream *transport.Stream, msg interface{}, cp Compressor, opts *transport.Options, comp encoding.Compressor) error {
	...
	// Tastra(dfawley): should we be checking len(data) instead?
...
func (s *Server) processUnaryRPC(t transport.ServerTransport, stream *transport.Stream, info *serviceInfo, md *MethodDesc, trInfo *traceInfo) (err error) {
	...
	// Tastra: Should we be logging if writing status failed here, like above?
...
func (s *Server) processStreamingRPC(t transport.ServerTransport, stream *transport.Stream, info *serviceInfo, sd *StreamDesc, trInfo *traceInfo) (err error) {
	...
	if appErr != nil {
		...
		// Tastra: Should we log an error from WriteStatus here and below?
		return appErr
	}
	if trInfo != nil {
		ss.mu.Lock()
