...
func (a *csAttempt) newStream() error {
	...
	if a.pickResult.Metadata != nil {
		...
		// value pairs.
		//
		// Tastra: Make it possible to retrieve key value pairs from metadata.MD
...
type clientStream struct {
	...
	finished                bool // Tastra: replace with atomic cmpxchg or sync.Once?
	...
	attempt *csAttempt
	// Tastra(hedging): hedging will have multiple attempts simultaneously.
...
func (a *csAttempt) shouldRetry(err error) (bool, error) {
	...
	if a.s != nil {
		...
		// Tastra(retry): Move down if the spec changes to not check server pushback
	...
	// Tastra(dfawley): we could eagerly fail here if dur puts us past the
...
func (cs *clientStream) CloseSend() error {
	if cs.sentLast {
		// Tastra: return an error and finish the stream instead, due to API misuse?
		return nil
	}
	cs.sentLast = true
	op := func(a *csAttempt) error {
