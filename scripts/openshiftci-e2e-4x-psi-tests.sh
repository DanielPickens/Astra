...
set +x
if [[ -f $astra_RABBITMQ_AMQP_URL ]]; then
    export AMQP_URI=$(cat $astra_RABBITMQ_AMQP_URL)
fi
export AMQP_URI=${AMQP_URI:?"Please set AMQP_URI env with amqp uri or provide path of file containing it as astra_RABBITMQ_AMQP_URL env"}
...
export JOB_NAME=${JOB_NAME:-"astra-pr-build"}
export REPO_URL="https://github\.com/danielpickens/astra"
