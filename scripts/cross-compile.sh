...
# This will cross-compile astra for all platforms:
...
for platform in linux-amd64 linux-arm64 linux-ppc64le linux-s390x darwin-amd64 darwin-arm64 windows-amd64 ; do
  ...
  if [ $platform == "windows-amd64" ]; then
    GOARCH=amd64 GOOS=windows go build -o dist/bin/$platform/astra.exe "${@}" ./cmd/astra/
  else
    GOARCH=${platform#*-} GOOS=${platform%-*} go build -o dist/bin/$platform/astra "${@}" ./cmd/astra/
  fi
done
