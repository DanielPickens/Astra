...
TEAM_PROVIDED=${TEAM_PROVIDED:-"astra"}
...
# PROJECT_AND_TIME is selected using labels app and team. e.g: app=test and team=astra
# eg. cmd-push-test157kgb|2021-08-17T12:40:20Z
PROJECT_AND_TIME=$(kubectl get namespace -o jsonpath='{range .items[*]}{.metadata.name}{"|"}{.metadata.creationTimestamp} {"\n"}{end}' | grep -v '^openshift\|^kube\|^default\|^ibm\|^calico\|^tigera\|^astra-operator-test')
