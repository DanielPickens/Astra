...
# this script updates version number in astra source code
...
sed -i "s/\(astra_VERSION:=\).*}/\1${NEW_VERSION}}/g" scripts/rpm-prepare.sh
...
for file in ./docs/website/docs/**/docs-mdx/**/*.mdx; do
  sed -i 's/\(astra version: v\).*/\1'"${NEW_VERSION}"'/g' "$file"
done

echo
echo "****************************************************************************************"
echo "* Don't forget to update homebrew package at https://github.com/kadel/homebrew-astra ! *"
echo "****************************************************************************************"
