...
var nodeJsSourcesAndDevfileFsContent = func(devfilePath string, options fsOptions) fscontentFunc {
	return func(fs filesystem.Filesystem) error {
		...
		helper.CopyExampleDevFile(
			...
			"devfile.yaml",
			"my-component")
		if options.dotastraExists || options.generated != nil {
			helper.MakeDir(util.DotastraDirectory)
		}
		if options.generated != nil {
			err := helper.CreateFileWithContent(filepath.Join(util.DotastraDirectory, "generated"), strings.Join(options.generated, "\n"))
		...
		for _, devstateFile := range options.devstateFiles {
			...
			dir := filepath.Dir(util.DotastraDirectory)
			err = os.MkdirAll(dir, 0750)
			if err != nil {
				return err
			}
			err = fs.WriteFile(fmt.Sprintf("./.astra/devstate.%d.json", devstateFile.PID), jsonContent, 0644)
			if err != nil {
				return err
			}
		}
...
var allFscontents = map[string]fscontentFunc{
	...
	"nodeJS sources, Devfile and .astra": nodeJsSourcesAndDevfileFsContent(
...
var checkCallsNonDeployedComponent = func(t *testing.T, clientset clientset.Clientset, testContext testContext) {
	...
	if strings.Contains(testContext.platform, "kubernetes") {
		...
		if testContext.runningInOption == "dev" {
			selector = selector + ",astra.dev/mode=Dev"
		} else if testContext.runningInOption == "deploy" {
			selector = selector + ",astra.dev/mode=Deploy"
		}
...
func TestastraDeleteMatrix(t *testing.T) {
	for _, tt := range []struct {
	...
	}{
		{
			...
			wantErr: "The current directory does not represent an astra component",
		},
		{
		...
		{
			name: "using --files in a directory where Devfile was not generated by astra",
			args: []string{"delete", "component", "-f"},
		...
		{
			name: "using --files in a directory where Devfile was generated by astra",
			args: []string{"delete", "component", "-f"},
