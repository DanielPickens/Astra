...
import (
	...
	"github.com/daniel-pickens/astra/pkg/astra/cli"
	"github.com/daniel-pickens/astra/pkg/astra/genericclioptions/clientset"
	"github.com/daniel-pickens/astra/pkg/astra/util"
)
...
`
	...
	return fmt.Sprintf(`= Overview of the astra CLI Structure
...
// Generates and returns a markdown-formatted CLI reference page for astra
func main() {
	var clidoc = &cobra.Command{
		Use:   "cli-doc",
		Short: "Generate CLI reference for astra",

		Example: `  # Generate a markdown-formatted CLI reference page for astra
  ...
  cli-doc structure`,
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			switch args[0] {
			case "reference":
				cmdastra, err := cli.NewCmdastra(ctx, cli.astraRecommendedName, cli.astraRecommendedName, func(err error) {
					util.LogErrorAndExit(err, "")
				}, testClientset)
				if err != nil {
					return err
				}
				fmt.Print(referencePrinter(cmdastra, 0))
			case "structure":
				cmdastra, err := cli.NewCmdastra(ctx, cli.astraRecommendedName, cli.astraRecommendedName, func(err error) {
					util.LogErrorAndExit(err, "")
				}, testClientset)
				...
				fmt.Print(commandPrinter(cmdastra, 0))
			default:
				fmt.Print(command.Usage())
			}
			return nil
