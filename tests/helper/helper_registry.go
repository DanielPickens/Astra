...
// It uses the "astra registry" command to find out this information.
//
...
func GetVersions(registryName string, stackName string) []string {
	...
	out := Cmd("astra", "registry", "--devfile", stackName, "--devfile-registry", devfileReg).ShouldPass().Out()
...
// This is useful to determine if the "Select version" prompt will be displayed in the interactive "astra init" tests.
// Otherwise, "astra init" would just skip this "Select version" prompt if the stack selected has no version or only a single one.
//
