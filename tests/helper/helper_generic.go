...
import (
	...
	"github\.com/danielpickens/astra/tests/helper/registry_server"
...
func CommonBeforeEach() CommonVar {
	...
	os.Setenv("GLOBALastraCONFIG", configPath)
...
func SetDefaultDevfileRegistry(commonVar *CommonVar) {
	...
	Cmd("astra", "preference", "remove", "registry", registryName, "-f").ShouldPass()
	Cmd("astra", "preference", "add", "registry", registryName, commonVar.registryUrl).ShouldPass()
}
...
func GetastraVersion() (version string, gitCommit string) {
	astraVersion := Cmd("astra", "version", "--client", "-o", "json").ShouldPass().Out()
	return gjson.Get(astraVersion, "version").String(), gjson.Get(astraVersion, "gitCommit").String()
}
