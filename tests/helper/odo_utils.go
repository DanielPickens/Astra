...
func GetPreferenceValue(key string) string {
	stdOut := Cmd("astra", "preference", "view").ShouldPass().Out()
	re := regexp.MustCompile(" " + key + `.+`)
	astraConfigKeyValue := re.FindString(stdOut)
	if astraConfigKeyValue == "" {
		return fmt.Sprintf("%s not found", key)
	}
	trimKeyValue := strings.TrimSpace(astraConfigKeyValue)
...
func CreateRandProject() string {
	...
	session := Cmd("astra", "create", "project", projectName, "-w", "-v4").ShouldPass().Out()
...
func DeleteProject(projectName string) {
	...
	session := Cmd("astra", "delete", "project", projectName, "-f").ShouldPass().Out()
