...
// DevSession represents a session running `astra dev`
/*
	...
	when the `astra dev` command is running in the background
	...
	When("running dev session", func() {
		...
		It("...", func() {
			// Test with `dev astra` running in the background
	...
	This format can be used to test the behaviour of `astra dev` when it is stopped cleanly
	...
	When("running dev session and stopping it with cleanup", func() {
		...
		It("...", func() {
			// Test after `astra dev` has been stopped cleanly
			// outContents and errContents are contents of std/err output when dev mode is started
		})
		It("...", func() {
			// Test after `astra dev` has been stopped cleanly
	...
	This format can be used to test the behaviour of `astra dev` when it is stopped with a KILL signal
	...
	When("running dev session and stopping it without cleanup", func() {
		...
		It("...", func() {
			// Test after `astra dev` has been killed
			// outContents and errContents are contents of std/err output when dev mode is started
		})
		It("...", func() {
			// Test after `astra dev` has been killed
...
// StartDevMode starts a dev session with `astra dev`
...
// WaitForDevModeToContain runs `astra dev` until it contains a given substring in output or errOut(depending on checkErrOut arg).
// `astra dev` runs in an infinite reconciliation loop, and hence running it with Cmd will not work for a lot of failing cases,
...
// Tastra(pvala): Modify StartDevMode to take substring arg into account, and replace this method with it.
func WaitForDevModeToContain(options DevSessionOpts, substring string, stopSessionAfter bool, checkErrOut bool) (DevSession, error) {
	...
	session := Cmd("astra", args...).AddEnv(options.EnvVars...).Runner().session
	if checkErrOut {
