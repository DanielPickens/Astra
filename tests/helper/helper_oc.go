...
func (oc OcRunner) SetProject(namespace string) string {
	...
	Cmd("astra", "set", "project", namespace).ShouldPass()
	return namespace
}
...
func (oc OcRunner) doAsAdmin(clusterType string) string {
	...
	if clusterType == "PSI" || clusterType == "IBM" {
		...
		if adminToken != "" {
			...
			// login ibmcloud
			Cmd("ibmcloud", "login", "--apikey", ibmcloudAdminToken, "-r", "us-east", "-g", "Developer-CI-and-QE")
...
func (oc OcRunner) doAsDeveloper(token, clusterType string) {
	...
	if clusterType == "IBM" {
		...
		Cmd("ibmcloud", "login", "--apikey", ibmcloudDeveloperToken, "-r", "us-east", "-g", "Developer-CI-and-QE")
...
func (oc OcRunner) addConfigMapForCleanup(projectName string) {
	Cmd(oc.path, "create", "configmap", "config-map-for-cleanup", "--from-literal", "type=testing", "--from-literal", "team=astra", "-n", projectName).ShouldPass()
}
