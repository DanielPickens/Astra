...
const (
	timePatternInastra = `(\[[0-9smh]+\])` // e.g. [4s], [1m], [3ms]
	staticTimeValue  = "[1s]"
...
var (
	...
	reastraVersion = regexp.MustCompile(`(v[0-9]+.[0-9]+.[0-9]+(?:-\w+)?)\s*\(\w+(-\w+)?\)`)
)

// ReplaceAllTimeInString replaces the time taken to download a Devfile or a starter project for an astra command with a custom value;
...
func ReplaceAllTimeInString(docString string, timeString string) string {
	reg := regexp.MustCompile(timePatternInastra)
	return reg.ReplaceAllString(docString, timeString)
}

// StripGitCommitFromVersion removes any git commit hash from the full astra version string.
...
func StripGitCommitFromVersion(docString string) string {
	return reastraVersion.ReplaceAllString(docString, "$1")
}
...
func StripSpinner(docString string) (returnString string) {
	...
	for sc.Scan() {
		...
		// this is specific to `astra dev` when it fails to pull images
