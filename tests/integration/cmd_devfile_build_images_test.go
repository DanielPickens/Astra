...
var _ = Describe("astra devfile build-images command tests", Label(helper.LabelSkipOnOpenShift), func() {
		...
		var _ = Context("label "+label, Label(label), func() {
			...
			When("using a devfile.yaml containing an Image component", func() {
				...
				BeforeEach(func() {
					...
					helper.Cmd("astra", "init", "--name", "aname", "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-outerloop.yaml")).ShouldPass()
				...
				for _, tt := range []struct {
					...
					{
						name:       "should run astra build-images without push",
					...
					{
						name:       "should run astra build-images --push",
						args:       []string{"--push"},
						env:        []string{"PODMAN_CMD=echo"},
						shouldPass: true,
						checkOutputFn: func(stdout, stderr string) {
					...
					},
					{
						...
						env: []string{
							"PODMAN_CMD=echo",
							"astra_IMAGE_BUILD_ARGS=--platform=linux/amd64;--build-arg=MY_ARG=my_value",
						},
						shouldPass: true,
						checkOutputFn: func(stdout, stderr string) {
					...
					It(tt.name, func() {
						...
						cmd := helper.Cmd("astra", args...).AddEnv(env...)
						if tt.shouldPass {
							cmd = cmd.ShouldPass()
						} else {
							cmd = cmd.ShouldFail()
			...
			When("using a devfile.yaml with no Image component", func() {
				...
				It("should not be able to run astra build-images", func() {
					stdout, stderr := helper.Cmd("astra", "build-images").AddEnv("PODMAN_CMD=echo").ShouldFail().OutAndErr()
			...
			When("using a devfile.yaml containing an Image component with Dockerfile args", func() {
				BeforeEach(func() {
					helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
					helper.Cmd("astra", "init", "--name", "aname", "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-outerloop-args.yaml")).ShouldPass()
					helper.CreateLocalEnv(commonVar.Context, "aname", commonVar.Project)
				})

				It("should use args to build image when running astra build-images", func() {
					stdout := helper.Cmd("astra", "build-images").AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
				...
				It("should be able pass extra flags to Podman/Docker build command", func() {
					stdout := helper.Cmd("astra", "build-images").AddEnv(
						"PODMAN_CMD=echo",
						"astra_IMAGE_BUILD_ARGS=--platform=linux/amd64;--build-arg=MY_ARG=my_value",
					).ShouldPass().Out()
			...
			When("using a devfile.yaml containing an Image component with a build context", func() {
				...
				for _, scope := range []struct {
					...
					It(fmt.Sprintf("should build image via %s if build context references PROJECT_SOURCE env var", scope.name), func() {
						stdout := helper.Cmd("astra", "build-images").AddEnv(scope.envvars...).ShouldPass().Out()
				...
				Describe("using a Devfile with an image component using a remote Dockerfile", func() {
					...
					When("remote server returns an error", func() {
						...
						It("should not build images", func() {
							cmdWrapper := helper.Cmd("astra", "build-images").AddEnv(env...).ShouldFail()
						...
						It("should not run 'astra build-images --push'", func() {
							cmdWrapper := helper.Cmd("astra", "build-images", "--push").AddEnv(env...).ShouldFail()
							stderr := cmdWrapper.Err()
							stdout := cmdWrapper.Out()
							Expect(stderr).To(ContainSubstring("failed to retrieve " + url))
							Expect(stdout).NotTo(ContainSubstring("build -t quay.io/unknown-account/myimage -f "))
...
`)
						...
						It("should build images", func() {
							stdout := helper.Cmd("astra", "build-images").AddEnv(env...).ShouldPass().Out()
						...
						It("should run 'astra build-images --push'", func() {
							stdout := helper.Cmd("astra", "build-images", "--push").AddEnv(env...).ShouldPass().Out()
							lines, _ := helper.ExtractLines(stdout)
							_, ok := helper.FindFirstElementIndexMatchingRegExp(lines, buildRegexp)
							Expect(ok).To(BeTrue(), "build regexp not found in output: "+buildRegexp)
							Expect(stdout).To(ContainSubstring("push quay.io/unknown-account/myimage"))
	...
	When("starting with Devfile with autoBuild or deployByDefault components",
		...
		func() {
			...
			When("building images", func() {
				...
				It("should build all Image components regardless of autoBuild", func() {
					for _, tag := range []string{
						...
						Expect(stdout).Should(ContainSubstring("Building Image: localhost:5000/astra-dev/node:%s", tag))
					}
				})
			})
			...
			When("building and pushing images", func() {
				...
				BeforeEach(func() {
					stdout = helper.Cmd("astra", "build-images", "--push").AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
				})
	...
	When("using a Devfile with variable image names", func() {
		...
		for _, push := range []bool{false, true} {
			...
			It(fmt.Sprintf("should build images with default variable values (push=%v)", push), func() {
				args := initialArgs
				stdout := helper.Cmd("astra", args...).AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
