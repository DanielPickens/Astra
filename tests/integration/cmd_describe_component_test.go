...
var _ = Describe("astra describe component command tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	for _, label := range []string{
		...
		It("should fail, without cluster", Label(label), func() {
			By("running astra describe component -o json with namespace flag without name flag", func() {
				res := helper.Cmd("astra", "describe", "component", "--namespace", "default", "-o", "json").ShouldFail()
			...
			By("running astra describe component -o json without name and without devfile in the current directory", func() {
				res := helper.Cmd("astra", "describe", "component", "-o", "json").ShouldFail()
				stdout, stderr := res.Out(), res.Err()
				Expect(helper.IsJSON(stderr)).To(BeTrue())
				Expect(stdout).To(BeEmpty())
				helper.JsonPathContentContain(stderr, "message", "The current directory does not represent an astra component")
			})

			By("running astra describe component with namespace flag without name flag", func() {
				res := helper.Cmd("astra", "describe", "component", "--namespace", "default").ShouldFail()
				stdout, stderr := res.Out(), res.Err()
				Expect(stdout).To(BeEmpty())
				Expect(stderr).To(ContainSubstring("--namespace can be used only with --name"))
			})

			By("running astra describe component without name and without devfile in the current directory", func() {
				res := helper.Cmd("astra", "describe", "component").ShouldFail()
				stdout, stderr := res.Out(), res.Err()
				Expect(stdout).To(BeEmpty())
				Expect(stderr).To(ContainSubstring("The current directory does not represent an astra component"))
			})
	...
	It("should fail, with default cluster mode", func() {
		By("running astra describe component -o json with an unknown name", func() {
			helper.CreateInvalidDevfile(commonVar.Context)
			res := helper.Cmd("astra", "describe", "component", "--name", "unknown-name", "-o", "json").ShouldFail()
			stdout, stderr := res.Out(), res.Err()
			Expect(helper.IsJSON(stderr)).To(BeTrue())
			Expect(stdout).To(BeEmpty())
		...
		By("running astra describe component with an unknown name", func() {
			helper.CreateInvalidDevfile(commonVar.Context)
			res := helper.Cmd("astra", "describe", "component", "--name", "unknown-name").ShouldFail()
			stdout, stderr := res.Out(), res.Err()
			Expect(stdout).To(BeEmpty())
	...
	It("should fail, with cluster", func() {
		By("running astra describe component -o json with an unknown name", func() {
			helper.CreateInvalidDevfile(commonVar.Context)
			res := helper.Cmd("astra", "describe", "component", "--name", "unknown-name", "--platform", "cluster", "-o", "json").ShouldFail()
			stdout, stderr := res.Out(), res.Err()
			Expect(helper.IsJSON(stderr)).To(BeTrue())
			Expect(stdout).To(BeEmpty())
		...
		By("running astra describe component with an unknown name", func() {
			helper.CreateInvalidDevfile(commonVar.Context)
			res := helper.Cmd("astra", "describe", "component", "--name", "unknown-name", "--platform", "cluster").ShouldFail()
			stdout, stderr := res.Out(), res.Err()
			Expect(stdout).To(BeEmpty())
	...
	It("should fail, with podman", Label(helper.LabelPodman), func() {
		By("running astra describe component -o json with an unknown name", func() {
			helper.CreateInvalidDevfile(commonVar.Context)
			res := helper.Cmd("astra", "describe", "component", "--name", "unknown-name", "--platform", "podman", "-o", "json").
				ShouldFail()
			stdout, stderr := res.Out(), res.Err()
			Expect(helper.IsJSON(stderr)).To(BeTrue())
			Expect(stdout).To(BeEmpty())
		...
		By("running astra describe component with an unknown name", func() {
			helper.CreateInvalidDevfile(commonVar.Context)
			stderr := helper.Cmd("astra", "describe", "component", "--name", "unknown-name", "--platform", "podman").
				ShouldFail().Err()
			Expect(stderr).To(ContainSubstring("no component found with name \"unknown-name\""))
		})
	})
	...
	When("creating a component", func() {
		BeforeEach(func() {
			...
			helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-registry.yaml")).ShouldPass()
		})
		...
		checkDevfileJSONDescription := func(jsonContent string, devfileName string) {
			...
			helper.JsonPathContentIs(jsonContent, "devfileData.supportedastraFeatures.dev", "true")
			helper.JsonPathContentIs(jsonContent, "devfileData.supportedastraFeatures.deploy", "false")
			helper.JsonPathContentIs(jsonContent, "devfileData.supportedastraFeatures.debug", "true")
			helper.JsonPathContentIs(jsonContent, "managedBy", "astra")
		...
		checkDevfileDescription := func(content string, withUnknown bool) {
			...
			Expect(content).To(ContainSubstring("Supported astra features:"))
			if withUnknown {
		...
		for _, label := range []string{
			...
			It("should describe the component in the current directory", Label(label), func() {
				By("running with json output", func() {
					res := helper.Cmd("astra", "describe", "component", "-o", "json").ShouldPass()
					stdout, stderr := res.Out(), res.Err()
				...
				By("running with default output", func() {
					res := helper.Cmd("astra", "describe", "component").ShouldPass()
					stdout := res.Out()
			...
			When("renaming devfile.yaml into a non-recognized file name", Label(label), func() {
				...
				It("should not describe the component in the current directory using the acceptable devfile filename", func() {
					...
					By("running with default output", func() {
						_, stderr := helper.Cmd("astra", "describe", "component").ShouldFail().OutAndErr()
						Expect(stderr).Should(ContainSubstring("The current directory does not represent an astra component"))
					})
				})
			})
		}
		...
		It("should not describe the component from another directory, with default cluster mode", func() {
			By("running with json output", func() {
				...
				res := helper.Cmd("astra", "describe", "component", "--name", cmpName, "-o", "json").ShouldFail()
				stdout, stderr := res.Out(), res.Err()
				Expect(helper.IsJSON(stderr)).To(BeTrue())
				Expect(stdout).To(BeEmpty())
			...
			By("running with default output", func() {
				...
				helper.Chdir(otherDir)
				helper.CreateInvalidDevfile(otherDir)
				res := helper.Cmd("astra", "describe", "component", "--name", cmpName).ShouldFail()
				stdout, stderr := res.Out(), res.Err()
				Expect(stdout).To(BeEmpty())
		...
		It("should not describe the component from another directory, with cluster", func() {
			By("running with json output", func() {
				...
				helper.Chdir(otherDir)
				helper.CreateInvalidDevfile(otherDir)
				res := helper.Cmd("astra", "describe", "component", "--name", cmpName, "-o", "json", "--platform", "cluster").ShouldFail()
				stdout, stderr := res.Out(), res.Err()
				Expect(helper.IsJSON(stderr)).To(BeTrue())
				Expect(stdout).To(BeEmpty())
			...
			By("running with default output", func() {
				...
				helper.Chdir(otherDir)
				helper.CreateInvalidDevfile(otherDir)
				res := helper.Cmd("astra", "describe", "component", "--name", cmpName, "--platform", "cluster").ShouldFail()
				stdout, stderr := res.Out(), res.Err()
				Expect(stdout).To(BeEmpty())
		...
		for _, podman := range []bool{true, false} {
			...
			for _, debug := range []bool{false, true} {
				debug := debug
				When(fmt.Sprintf("running astra dev (podman=%s,debug=%s)", strconv.FormatBool(podman), strconv.FormatBool(debug)), helper.LabelPodmanIf(podman, func() {
					...
					BeforeEach(func() {
						...
						if debug {
							...
							if podman {
								// Tastra(rm3l): use forward-localhost when it is implemented
					...
					Context("Default output", func() {
						...
						When("describing the component in dev mode", func() {
							var stdout string
							BeforeEach(func() {
								stdout = helper.Cmd("astra", "describe", "component").ShouldPass().Out()
							})
						...
						When("switching to another directory", func() {
							...
							When("describing the component from another directory", func() {
								var stdout string
								BeforeEach(func() {
									stdout = helper.Cmd("astra", "describe", "component", "--name", cmpName).
										ShouldPass().
										Out()
								})
					...
					Context("JSON output", func() {
						...
						When("describing the component in dev mode", func() {
							...
							BeforeEach(func() {
								stdout, stderr = helper.Cmd("astra", "describe", "component", "-o", "json").
									ShouldPass().
									OutAndErr()
							})
						...
						When("switching to another directory", func() {
							...
							When("describing the component from another directory", func() {
								var stdout, stderr string
								BeforeEach(func() {
									stdout, stderr = helper.Cmd("astra", "describe", "component", "--name", cmpName, "-o", "json").
										ShouldPass().
										OutAndErr()
								})
		...
		for _, ctx := range []struct {
			...
			ctx := ctx
			When("running astra deploy to create ingress/routes", func() {
				...
				It(fmt.Sprintf("should show the %s in astra describe component output", ctx.title), func() {
					By("checking the human readable output", func() {
						out := helper.Cmd("astra", "describe", "component").ShouldPass().Out()
						helper.MatchAllInOutput(out, ctx.matchOutput)
					})
					By("checking the machine readable output", func() {
						out := helper.Cmd("astra", "describe", "component", "-o", "json").ShouldPass().Out()
					...
					By("checking the human readable output with component name", func() {
						helper.CreateInvalidDevfile(commonVar.Context)
						out := helper.Cmd("astra", "describe", "component", "--name", componentName).ShouldPass().Out()
						helper.MatchAllInOutput(out, ctx.matchOutput)
					})
					By("checking the machine readable output with component name", func() {
						helper.CreateInvalidDevfile(commonVar.Context)
						out := helper.Cmd("astra", "describe", "component", "--name", componentName, "-o", "json").ShouldPass().Out()
						for key, value := range ctx.matchJSONOutput {
							helper.JsonPathContentContain(out, key, value)
						}
					})
	...
	Context("checking for remote source code location", func() {
		for _, podman := range []bool{true, false} {
			...
			for _, ctx := range []struct {
				...
				ctx := ctx
				When(fmt.Sprintf("using %s and starting an astra dev session", ctx.title), helper.LabelPodmanIf(podman, func() {
					var devSession helper.DevSession
					BeforeEach(func() {
						helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
						helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", ctx.devfile)).ShouldPass()
						var err error
					...
					It("should show remote source code location in astra describe component output", func() {
	...
	When("a non-astra application is present on the cluster", func() {
		var (
			// From manifests
		...
		It("should describe the component", func() {
			output := helper.Cmd("astra", "describe", "component", "--name", componentName).ShouldPass().Out()
	...
	Context("describe commands in Devfile", Label(helper.LabelUnauth), Label(helper.LabelNoCluster), func() {
		...
		When("initializing a component with different types of commands", func() {
			...
			It("should describe the Devfile commands in JSON output", func() {
				stdout := helper.Cmd("astra", "describe", "component", "-o", "json").ShouldPass().Out()
