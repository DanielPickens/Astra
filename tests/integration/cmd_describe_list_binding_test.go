...
var _ = Describe("astra describe/list binding command tests", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
	...
	for _, ns := range []string{"", fmt.Sprintf("binding-%s", helper.RandString(3))} {
		...
		When(fmt.Sprintf("creating a component with a binding (service in namespace %q)", ns), func() {
			...
			BeforeEach(func() {
				...
				helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-with-service-binding-files.yaml")).ShouldPass()
				if ns != "" {
          ...
          namespace: %s`, ns))
			...
			for _, label := range []string{
				...
				label := label
				It("should describe the binding without running astra dev", Label(label), func() {
					By("JSON output", func() {
						res := helper.Cmd("astra", "describe", "binding", "-o", "json").ShouldPass()
					...
					By("human readable output", func() {
						res := helper.Cmd("astra", "describe", "binding").ShouldPass()
			...
			for _, command := range [][]string{
				...
				command := command
				It(fmt.Sprintf("should list the binding without running astra dev (%s)", command), func() {
					By("JSON output", func() {
						res := helper.Cmd("astra", append(command, "-o", "json")...).ShouldPass()
						stdout, stderr := res.Out(), res.Err()
						Expect(stderr).To(BeEmpty())
						Expect(helper.IsJSON(stdout)).To(BeTrue())
					...
					By("human readable output", func() {
						res := helper.Cmd("astra", command...).ShouldPass()
						stdout, _ := res.Out(), res.Err()
		...
		for _, ctx := range []struct {
			...
			When(fmt.Sprintf("%s (service in namespace %q)", ctx.title, ns), func() {
				...
				BeforeEach(func() {
					helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
					helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", ctx.devfile).ShouldPass()
				})
					...
					When("running dev session", func() {
						...
						It("should describe the binding", func() {
							...
							By("JSON output from another directory with name flag and invalid devfile", func() {
								...
								res := helper.Cmd("astra", "describe", "binding", "--name", "my-nodejs-app-cluster-sample-k8s", "-o", "json").ShouldPass()
								stdout, stderr := res.Out(), res.Err()
								ctx.assertDescribeNamedJsonOutput(stdout, stderr, "my-nodejs-app-cluster-sample-k8s")

								res = helper.Cmd("astra", "describe", "binding", "--name", "my-nodejs-app-cluster-sample-ocp", "-o", "json").ShouldPass()
								stdout, stderr = res.Out(), res.Err()
							...
							By("human readable output from another directory with name flag and invalid devfile", func() {
								...
								helper.Chdir(otherDir)
								helper.CreateInvalidDevfile(otherDir)
								res := helper.Cmd("astra", "describe", "binding", "--name", "my-nodejs-app-cluster-sample-k8s").ShouldPass()
								stdout, stderr := res.Out(), res.Err()
								ctx.assertDescribeNamedHumanReadableOutput(stdout, stderr, "my-nodejs-app-cluster-sample-k8s")

								res = helper.Cmd("astra", "describe", "binding", "--name", "my-nodejs-app-cluster-sample-ocp").ShouldPass()
								stdout, stderr = res.Out(), res.Err()
							...
							When("changing the current namespace", func() {
								...
								It(fmt.Sprintf("should list the binding with --namespace flag - command: %s", command), func() {
									By("JSON output from another directory", func() {
										err := os.Chdir("/")
										Expect(err).ToNot(HaveOccurred())
										res := helper.Cmd("astra", append(command, "-o", "json", "--namespace", commonVar.Project)...).ShouldPass()
										stdout, stderr := res.Out(), res.Err()
										if ctx.assertListJsonOutput != nil {
											ctx.assertListJsonOutput(false, stdout, stderr)
										}
									...
									By("human readable output from another directory with name flag", func() {
										err := os.Chdir("/")
										Expect(err).ToNot(HaveOccurred())
										res := helper.Cmd("astra", append(command, "--namespace", commonVar.Project)...).ShouldPass()
										stdout, stderr := res.Out(), res.Err()
										if ctx.assertListHumanReadableOutput != nil {
											ctx.assertListHumanReadableOutput(false, stdout, stderr, command)
										}
