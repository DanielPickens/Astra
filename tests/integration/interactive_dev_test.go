...
var _ = Describe("astra dev interactive command tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	Context("directory is not empty", func() {
		...
		When("there is a match from Alizer", func() {
			...
			It("should run alizer to download devfile successfully even with -v flag", func() {
				...
				_, _ = helper.RunInteractive([]string{"astra", "dev", "--random-ports", "-v", "4"},
					nil,
			...
			It("should run alizer to download devfile", func() {
				...
				projectType := "Python"
				versionedDevfileName := "python:2.1.0"
				_, _ = helper.RunInteractive([]string{"astra", "dev", "--random-ports"},
					nil,
					func(ctx helper.InteractiveContext) {
						helper.ExpectString(ctx, "Based on the files in the current directory astra detected")
			...
			It("should display welcoming messages first", func() {
				...
				projectType := "Python"
				versionedDevfileName := "python:2.1.0"
				output, _ := helper.RunInteractive([]string{"astra", "dev", "--random-ports"},
		...
		When("Alizer cannot determine a Devfile based on the current source code", func() {
			...
			It("should not fail but fallback to the interactive mode", func() {
				output, _ := helper.RunInteractive([]string{"astra", "dev", "--random-ports"}, nil, func(ctx helper.InteractiveContext) {
	...
	When("a component is bootstrapped", func() {
		...
		It("should sync files when p is pressed", func() {
			_, _ = helper.RunInteractive([]string{"astra", "dev", "--random-ports", "--no-watch"},
				nil,
				func(ctx helper.InteractiveContext) {
