...
var _ = Describe("astra devfile deploy command tests", func() {
	...
	When("directory is empty", func() {
		...
		It("should error", func() {
			output := helper.Cmd("astra", "deploy").ShouldFail().Err()
	...
	When("a component is bootstrapped", func() {
		BeforeEach(func() {
			...
			helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-deploy.yaml")).ShouldPass()
		})
		When("using a default namespace", func() {
			...
			AfterEach(func() {
				helper.Cmd("astra", "delete", "component", "-f").ShouldPass()
				commonVar.CliRunner.SetProject(commonVar.Project)
			})

			It("should display warning when running the deploy command", func() {
				errOut := helper.Cmd("astra", "deploy").AddEnv("PODMAN_CMD=echo").ShouldRun().Err()
				namespace := "project"
				if helper.IsKubernetesCluster() {
					namespace = "namespace"
				}
				Expect(errOut).To(ContainSubstring(fmt.Sprintf("You are using \"default\" %[1]s, astra may not work as expected in the default %[1]s.", namespace)))
			})
		})
		It("should fail to run astra deploy when not connected to any cluster", Label(helper.LabelNoCluster), func() {
			errOut := helper.Cmd("astra", "deploy").ShouldFail().Err()
	...
	for _, ctx := range []struct {
		...
		When(ctx.title, func() {
			...
			for _, tt := range []struct {
				...
				{
					name: "running astra deploy",
				},
				{
					name: "running astra deploy with image build extra args",
					imageBuildExtraArgs: []string{
				...
				When(tt.name, func() {
					...
					BeforeEach(func() {
						...
						stdout = helper.Cmd("astra", "deploy").AddEnv(env...).ShouldPass().Out()
					})
					...
					It("should run astra dev successfully", func() {
					...
					When("running and stopping astra dev", func() {
						BeforeEach(func() {
							devSession, err := helper.StartDevMode(helper.DevSessionOpts{})
						...
						It("should not delete the resources created with astra deploy", func() {
...
`)
				...
				When("running astra deploy", func() {
					var stdout string
					BeforeEach(func() {
...
`)
	...
	When("using a devfile.yaml containing two deploy commands", func() {
		...
		It("should run astra deploy", func() {
			stdout := helper.Cmd("astra", "deploy").AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
			By("building and pushing image to registry", func() {
	...
	When("recording telemetry data", func() {
		...
		It("should record the telemetry data correctly", func() {
			...
			Expect(td.Event).To(ContainSubstring("astra deploy"))
	...
	When("using a devfile.yaml containing an Image component with a build context", func() {
		...
		BeforeEach(func() {
			helper.CopyExample(filepath.Join("source", "nodejs"), commonVar.Context)
			helper.Cmd("astra", "init", "--name", "aname",
				"--devfile-path",
		...
		for _, scope := range []struct {
			...
			It(fmt.Sprintf("should build image via %s if build context references PROJECT_SOURCE env var", scope.name), func() {
				stdout := helper.Cmd("astra", "deploy").AddEnv(scope.envvars...).ShouldPass().Out()
		...
		When(fmt.Sprintf("deploying a Devfile %s component with multiple K8s resources defined", ctx.componentType), func() {
			...
			BeforeEach(func() {
				...
				helper.CopyExampleDevFile(
					...
					cmpName)
				out = helper.Cmd("astra", "deploy").AddEnv("PODMAN_CMD=echo").ShouldPass().Out()
		...
		Describe("using a Devfile with an image component using a remote Dockerfile", func() {
			...
			When("remote server returns an error", func() {
				...
				It("should not build images", func() {
					cmdWrapper := helper.Cmd("astra", "deploy").AddEnv(env...).ShouldFail()
...
`)
	...
	Context("deploying devfile with exec", func() {
		...
		for _, ctx := range []struct {
		...
		}{
			{
				...
				compName: "document-how-astra-translates-container-component-to-deploymentss",
			},
			{
			...
			When(fmt.Sprintf("using devfile that works; with %s", ctx.title), func() {
				...
				It("should complete the command execution successfully", func() {
					out := helper.Cmd("astra", "deploy").ShouldPass().Out()
			...
			// the job name will be truncated, and we want to ensure the correct truncated name is used to delete the old job before running a new one so that `astra deploy` does not fail
			When(fmt.Sprintf("the deploy command terminates abruptly; %s", ctx.title), func() {
				BeforeEach(func() {
					...
					helper.Cmd("astra", "deploy").WithTimeout(10).ShouldFail()
				})
				When("astra deploy command is run again", func() {
					BeforeEach(func() {
			...
			It("should not set securitycontext for podsecurity admission on job's pod template", func() {
				...
				helper.Cmd("astra", "deploy").Should(func(session *gexec.Session) {
		...
		When("using a devfile name with length more than 63", func() {
			const (
				unacceptableLongName = "document-how-astra-translates-container-component-to-deploymentsss"
			)
			BeforeEach(func() {
		...
		When("using devfile with a long running command in exec", func() {
			...
			It("should print the tip to run astra logs after 1 minute of execution", func() {
				out := helper.Cmd("astra", "deploy").ShouldPass().Out()
				Expect(out).To(ContainSubstring("Tip: Run `astra logs --deploy --follow` to get the logs of the command output."))
			})
		})
		...
		When("using devfile where the exec command is bound to fail", func() {
			...
			It("should print the last 100 lines of the log to the output", func() {
				out, errOut := helper.Cmd("astra", "deploy").ShouldFail().OutAndErr()
		...
		When("Automount volumes are present in the namespace", func() {
			...
			It("should mount the volumes", func() {
				helper.Cmd("astra", "deploy").Should(func(session *gexec.Session) {
					...
					// More tests are executed on `astra dev`, see "Automount volumes are present in the namespace" on astra dev tests.
	...
	Context("Devfile with autoBuild or deployByDefault components", func() {
		...
		When("starting with Devfile with Deploy commands", func() {
			...
			When("running astra deploy with some components not referenced in the Devfile", func() {
				var stdout string
				...
				It("should create the appropriate resources", func() {
					...
					By("automatically applying image components with autoBuild=true", func() {
						for _, tag := range []string{
							...
							Expect(stdout).Should(ContainSubstring("Building & Pushing Image: localhost:5000/astra-dev/node:%s", tag))
						}
					})
					By("automatically applying non-referenced Image components with autoBuild not set", func() {
						Expect(stdout).Should(ContainSubstring("Building & Pushing Image: localhost:5000/astra-dev/node:autobuild-not-set-and-not-referenced"))
					})
					By("not applying image components with autoBuild=false", func() {
						for _, tag := range []string{
							...
							Expect(stdout).ShouldNot(ContainSubstring("localhost:5000/astra-dev/node:%s", tag))
						}
					})
					By("not applying referenced Image components with deployByDefault unset", func() {
						Expect(stdout).ShouldNot(ContainSubstring("localhost:5000/astra-dev/node:autobuild-not-set-and-referenced"))
					})
				})
			})

			When("running astra deploy with some components referenced in the Devfile", func() {
				var stdout string

				BeforeEach(func() {
					//Tastra (rm3l): we do not support passing a custom deploy command yet. That's why we are manually updating the Devfile to set the default deploy command.
				...
				It("should create the appropriate resources", func() {
					...
					By("not applying non-referenced image components with autoBuild=false", func() {
						Expect(stdout).ShouldNot(ContainSubstring("localhost:5000/astra-dev/node:autobuild-false-and-not-referenced"))
					})
				})
			})
		...
		When("starting with Devfile with no Deploy command", func() {
			...
			It("should fail to run astra deploy", func() {
				stdout, stderr := helper.Cmd("astra", "deploy").AddEnv("PODMAN_CMD=echo").ShouldFail().OutAndErr()
				...
				By("not automatically applying Image components ", func() {
					for _, s := range []string{stdout, stderr} {
						Expect(s).ShouldNot(ContainSubstring("Building & Pushing Image: localhost:5000/astra-dev/node:"))
					}
				})
