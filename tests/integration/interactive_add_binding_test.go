...
var _ = Describe("astra add binding interactive command tests", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
	...
	When("the component is bootstrapped", func() {
		...
		BeforeEach(func() {
			helper.Cmd("astra", "init", "--name", componentName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile.yaml"), "--starter", "nodejs-starter").ShouldPass()
		...
		It("should successfully add binding to the devfile (Bind as Environment Variables)", func() {
			command := []string{"astra", "add", "binding"}
			...
			_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
				...
				helper.ExpectString(ctx, fmt.Sprintf("astra add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s --bind-as-files=false", bindingName))
			})
		...
		It("should successfully add binding to the devfile (Bind as Files)", func() {
			...
			_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
				...
				helper.ExpectString(ctx, fmt.Sprintf("astra add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s", bindingName))
			})
			...
			It(fmt.Sprintf("should successfully add binding to the devfile (%q as naming strategy)", predefinedNamingStrategy), func() {
				...
				_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
					...
					helper.ExpectString(ctx,
						fmt.Sprintf("astra add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s --bind-as-files=false --naming-strategy='%s'",
		...
		for _, tt := range []struct {
			...
			It(fmt.Sprintf("should successfully add binding to the devfile (custom naming strategy: %q)", tt.namingStrategy), func() {
				...
				_, err := helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
					...
					automationMsg := fmt.Sprintf("astra add binding --service cluster-sample.Cluster.postgresql.k8s.enterprisedb.io --name %s", bindingName)
		...
		When("binding to a service in a different namespace", func() {
			...
			It("should error out if service is not found in the namespace selected", func() {
				_, err := helper.RunInteractive([]string{"astra", "add", "binding"}, nil, func(ctx helper.InteractiveContext) {
					helper.ExpectString(ctx, "Do you want to list services from:")
