...
import (
	...
	"github\.com/danielpickens/astra/pkg/remotecmd"
...
var _ = Describe("astra dev command tests", Label(helper.LabelSkipOnOpenShift), func() {
	var cmpName string
	...
	When("directory is empty", func() {
		...
		It("should error", func() {
			output := helper.Cmd("astra", "dev", "--random-ports").ShouldFail().Err()
	...
	When("a component is bootstrapped", func() {
		BeforeEach(func() {
			...
			helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", devfile)).ShouldPass()
			Expect(helper.VerifyFileExists(".astra/env/env.yaml")).To(BeFalse())
		})

		It("should fail to run astra dev when not connected to any cluster", Label(helper.LabelNoCluster), func() {
			errOut := helper.Cmd("astra", "dev").ShouldFail().Err()
			Expect(errOut).To(ContainSubstring("unable to access the cluster"))
		})
		It("should fail to run astra dev when podman is nil", Label(helper.LabelPodman), func() {
			errOut := helper.Cmd("astra", "dev", "--platform", "podman").WithEnv("PODMAN_CMD=echo").ShouldFail().Err()
		...
		It("should start on cluster even if Podman client takes long to initialize", func() {
			...
			// astra dev on cluster should not wait for the Podman client to initialize properly, if this client takes very long.
			// See https://github\.com/danielpickens/astra/issues/6575.
		...
		When("using a default namespace", func() {
			...
			It("should print warning about default namespace when running astra dev", func() {
				namespace := "project"
				...
				Expect(devSession.ErrOut).To(ContainSubstring(fmt.Sprintf("You are using \"default\" %[1]s, astra may not work as expected in the default %[1]s.", namespace)))
			})
		})
		...
		It("ensure that index information is updated", func() {
			err := helper.RunDevMode(helper.DevSessionOpts{}, func(session *gexec.Session, outContents, errContents string, ports map[string]string) {
				indexAfterPush, err := util.ReadFileIndex(filepath.Join(commonVar.Context, ".astra", "astra-file-index.json"))
				Expect(err).ToNot(HaveOccurred())
				...
				Eventually(func() bool {
					...
					newIndexAfterPush, readErr := util.ReadFileIndex(filepath.Join(commonVar.Context, ".astra", "astra-file-index.json"))
					if readErr != nil {
				...
				Eventually(func() bool {
					...
					newIndexAfterPush, err := util.ReadFileIndex(filepath.Join(commonVar.Context, ".astra", "astra-file-index.json"))
					if err != nil {
		...
		When("a state file is not writable", func() {
			BeforeEach(func() {
				stateFile := filepath.Join(commonVar.Context, ".astra", "devstate.json")
			...
			It("should fail running astra dev", func() {
				res := helper.Cmd("astra", "dev", "--random-ports").ShouldFail()
				stdout := res.Out()
				stderr := res.Err()
		...
		for _, podman := range []bool{true, false} {
			...
			When("recording telemetry data", helper.LabelPodmanIf(podman, func() {
				...
				It("should record the telemetry data correctly", func() {
					...
					Expect(td.Event).To(ContainSubstring("astra dev"))
			...
			When("astra dev is executed", helper.LabelPodmanIf(podman, func() {
				...
				When("astra dev is stopped", func() {
					BeforeEach(func() {
						devSession.Stop()
						devSession.WaitEnd()
					})
		...
		When("an env.yaml file contains a non-current Project", func() {
			BeforeEach(func() {
				...
				err := helper.CreateFileWithContent(filepath.Join(astraDir, "env.yaml"), `
ComponentSettings:
...
`)
			...
			When("astra dev is executed", func() {
				...
				It("should not have modified env.yaml, and use current namespace", func() {
					helper.FileShouldContainSubstring(".astra/env/env.yaml", "Project: another-project")
		...
		When("astra dev is executed and Ephemeral is set to false", func() {
			...
			BeforeEach(func() {
				...
				helper.Cmd("astra", "preference", "set", "-f", "Ephemeral", "false").ShouldPass()
				var err error
				devSession, err = helper.StartDevMode(helper.DevSessionOpts{})
				Expect(err).ToNot(HaveOccurred())
			})

			When("killing astra dev and running astra delete component --wait", func() {
				BeforeEach(func() {
					devSession.Kill()
					devSession.WaitEnd()
					helper.Cmd("astra", "delete", "component", "--wait", "-f").ShouldPass()
				})
			...
			When("killing astra dev and another process replaces it", func() {
				var newDevSession helper.DevSession
				BeforeEach(func() {
					pid := devSession.PID()
					devSession.Kill()
					devSession.WaitEnd()
					devstate := fmt.Sprintf(".astra/devstate.%d.json", pid)
					newdevstate := fmt.Sprintf(".astra/devstate.%d.json", helper.GetFirstProcess())
				...
				It("should restart a new session successfully", func() {
					...
					Expect(err).ToNot(HaveOccurred())
					logMsg := fmt.Sprintf("process %d exists but is not astra, ignoring", helper.GetFirstProcess())
			...
			When("stopping astra dev normally", func() {
				BeforeEach(func() {
					devSession.Stop()
					devSession.WaitEnd()
				})
		...
		for _, podman := range []bool{true, false} {
			...
			for _, noCommandsFlag := range []string{"", "--no-commands", "--no-commands=false"} {
				...
				When("astra is executed with --no-watch flag and "+suffix, helper.LabelPodmanIf(podman, func() {
		...
		When("a delay is necessary for the component to start and running astra dev", func() {
			...
			BeforeEach(func() {
				helper.ReplaceString(filepath.Join(commonVar.Context, "devfile.yaml"),
					"npm start",
					// astra dev now waits some time until the app is ready or a timeout (current set to 1m) expires before starting port-forwarding.
					// So we are sleeping more than the timeout.
					// See https://github\.com/danielpickens/astra/issues/6667
					"sleep 80 ; npm start")
		...
		for _, podman := range []bool{true, false} {
			...
			When("build command takes really long to start", helper.LabelPodmanIf(podman, func() {
				...
				It("should cancel build command and return if astra dev is stopped", func() {
					opts := helper.DevSessionOpts{
						RunOnPodman: podman,
					}
			...
			When("run command takes really long to start", helper.LabelPodmanIf(podman, func() {
				...
				It("should cancel run command and return if astra dev is stopped", func() {
					opts := helper.DevSessionOpts{
						RunOnPodman: podman,
					}
	...
	Context("checking if astra dev matches local Devfile K8s resources and remote resources", func() {
		for _, devfile := range []struct {
			...
			devfile := devfile
			When(fmt.Sprintf("astra dev is executed to run a devfile containing a k8s resource %s", devfile.title), func() {
				var (
				...
				BeforeEach(
					func() {
						...
						// ensure the deployment is created by `astra dev`
	...
	for _, ctx := range []struct {
		...
		{
			title:          "astra dev is executed to run a devfile containing a k8s resource",
			devfile:        "devfile-with-k8s-resource.yaml",
			matchResources: []string{"my-component"},
		},
		{
			title:          "astra dev is executed to run a devfile containing multiple k8s resource defined under a single Devfile component",
	...
	for _, podman := range []bool{true, false} {
		...
		Context(fmt.Sprintf("multiple dev sessions with different project are running on same platform (podman=%v), same port", podman), helper.LabelPodmanIf(podman, func() {
			...
			When("astra dev session is run for nodejs component", func() {
				BeforeEach(func() {
				...
				When("astra dev session is run for go project on the same port but different address", func() {
					BeforeEach(func() {
			...
			for _, manual := range []bool{true, false} {
				...
				When("devfile has no endpoint", func() {
					BeforeEach(func() {
						...
						helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
						helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-no-endpoint.yaml")).ShouldPass()
					})

					When("running astra dev", func() {
						var devSession helper.DevSession
						BeforeEach(func() {
							var err error
							opts := []string{}
					...
					for _, customPortForwarding := range []bool{true, false} {
						...
						When("devfile has single endpoint", func() {
							...
							BeforeEach(func() {
								...
								helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
								helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile.yaml")).ShouldPass()
							})
							...
							When("running astra dev", func() {
								...
								When("modifying name for container in Devfile", func() {
									...
									It(fmt.Sprintf("should react on the Devfile modification (podman=%v, manual=%v, customPortForwarding=%v, customAddress=%v)", podman, manual, customPortForwarding, customAddress), func() {
										By("not warning users that astra dev needs to be restarted", func() {
											warning := "Please restart 'astra dev'"
						...
						When("devfile has multiple endpoints", func() {
							...
							BeforeEach(func() {
								...
								helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-with-multiple-endpoints.yaml")).ShouldPass()
							})
							...
							When("running astra dev", func() {
								...
								It(fmt.Sprintf("should expose all endpoints on localhost regardless of exposure(podman=%v, manual=%v, customPortForwarding=%v, customAddress=%v)", podman, manual, customPortForwarding, customAddress), func() {
									...
									By("not warning users that astra dev needs to be restarted because the Devfile has not changed", func() {
										warning := "Please restart 'astra dev'"
										Expect(devSession.StdOut).ShouldNot(ContainSubstring(warning))
										Expect(devSession.ErrOut).ShouldNot(ContainSubstring(warning))
									})
	...
	for _, devfileHandlerCtx := range []struct {
		...
		for _, podman := range []bool{true, false} {
			...
			When("Devfile 2.1.0 is used - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				...
				When("doing astra dev with --var flag", func() {
					var devSession helper.DevSession
					BeforeEach(func() {
						var err error
						devSession, err = helper.StartDevMode(helper.DevSessionOpts{
				...
				When("doing astra dev with --var-file flag", func() {
					var devSession helper.DevSession
				...
				When("doing astra dev with --var-file flag and setting value in env", func() {
					var devSession helper.DevSession
					varfilename := "vars.txt"
					BeforeEach(func() {
						var err error
			...
			When("running astra dev and single env var is set - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				var devfileCmpName string
				BeforeEach(func() {
					devfileCmpName = helper.RandString(6)
					helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
			...
			When("running astra dev and multiple env variables are set - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				var devfileCmpName string
				BeforeEach(func() {
					devfileCmpName = helper.RandString(6)
					helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
			...
			When("doing astra dev and there is a env variable with spaces - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				var devfileCmpName string
				BeforeEach(func() {
					devfileCmpName = helper.RandString(6)
					helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
		...
		When("creating local files and dir and running astra dev - "+devfileHandlerCtx.name, func() {
		...
		When("adding local files to gitignore and running astra dev", func() {
...
!main.css`); err != nil {
		...
		When("devfile has sourcemappings and running astra dev - "+devfileHandlerCtx.name, func() {
			var devfileCmpName string
			var devSession helper.DevSession
			BeforeEach(func() {
				devfileCmpName = helper.RandString(6)
		...
		When("project and clonePath is present in devfile and running astra dev - "+devfileHandlerCtx.name, func() {
			var devfileCmpName string
			var devSession helper.DevSession
			BeforeEach(func() {
				devfileCmpName = helper.RandString(6)
		...
		When("devfile project field is present and running astra dev - "+devfileHandlerCtx.name, func() {
			var devfileCmpName string
			var devSession helper.DevSession
			BeforeEach(func() {
				devfileCmpName = helper.RandString(6)
		...
		When("running astra dev with devfile contain volume - "+devfileHandlerCtx.name, func() {
			var devfileCmpName string
			var devSession helper.DevSession
			BeforeEach(func() {
				devfileCmpName = helper.RandString(6)
		...
		When("running astra dev with devfile containing volume-component - "+devfileHandlerCtx.name, func() {
			var devfileCmpName string
			var devSession helper.DevSession
			BeforeEach(func() {
				if os.Getenv("KUBERNETES") == "true" {
	...
	Describe("1. devfile contains composite apply command", func() {
		...
		for _, tt := range []struct {
			...
			{
				name: "astra dev is running",
			},
			{
				name: "astra dev is running with image build extra args",
				imageBuildExtraArgs: []string{
			...
			{
				name: "astra dev is running with container backend global extra args",
				containerBackendGlobalExtraArgs: []string{
					"--log-level=error",
				},
			},
			{
				name: "astra dev is running with container run extra args",
				containerRunExtraArgs: []string{
					"--quiet",
					"--tls-verify=false",
				},
			...
			{
				name: "astra dev is running with both image build and container run extra args",
				imageBuildExtraArgs: []string{
					"--platform=linux/amd64",
					"--build-arg=MY_ARG=my_value",
				},
			...
			for _, podman := range []bool{false, true} {
				...
				When(tt.name, helper.LabelPodmanIf(podman, func() {
					BeforeEach(func() {
						...
						var env []string
						if podman {
							env = append(env, "astra_PUSH_IMAGES=false")
						} else {
							env = append(env, "PODMAN_CMD=echo")
						}
						if len(tt.containerBackendGlobalExtraArgs) != 0 {
							env = append(env, "astra_CONTAINER_BACKEND_GLOBAL_ARGS="+strings.Join(tt.containerBackendGlobalExtraArgs, ";"))
						}
						if len(tt.imageBuildExtraArgs) != 0 {
							env = append(env, "astra_IMAGE_BUILD_ARGS="+strings.Join(tt.imageBuildExtraArgs, ";"))
						}
						var cmdLineArgs []string
						if len(tt.containerRunExtraArgs) != 0 {
							env = append(env, "astra_CONTAINER_RUN_ARGS="+strings.Join(tt.containerRunExtraArgs, ";"))
						}
						if podman {
		...
		Context("the devfile contains an image component that uses a remote Dockerfile", func() {
			...
			for _, env := range [][]string{
				{"PODMAN_CMD=echo"},
				{
					"PODMAN_CMD=a-command-not-found-for-podman-should-make-astra-fallback-to-docker",
					"DOCKER_CMD=echo",
				},
			} {
				env := env
				When(fmt.Sprintf("%v remote server returns a valid file when astra dev is run", env), func() {
...
`)
					...
					It("should build and push image when astra dev is run", func() {
				...
				When(fmt.Sprintf("%v remote server returns an error when astra dev is run", env), func() {
					var server *httptest.Server
					var url string
					BeforeEach(func() {
						server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					It("should not build images when astra dev is run", func() {
	...
	for _, devfileHandlerCtx := range []struct {
		...
		for _, podman := range []bool{true, false} {
			...
			When("running astra dev and devfile with composite command - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				var devfileCmpName string
				var devSession helper.DevSession
				BeforeEach(func() {
					devfileCmpName = helper.RandString(6)
			...
			When("running astra dev and composite command is marked as parallel:true - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				var devfileCmpName string
				var devSession helper.DevSession
				BeforeEach(func() {
					devfileCmpName = helper.RandString(6)
			...
			When("running astra dev and composite command are nested - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				var devfileCmpName string
				var devSession helper.DevSession
				BeforeEach(func() {
					devfileCmpName = helper.RandString(6)
			...
			When("running astra dev and composite command is used as a run command - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				var devSession helper.DevSession
				var devfileCmpName string
				BeforeEach(func() {
					devfileCmpName = helper.RandString(6)
				...
				It("should run successfully", func() {
					By("telling the user that astra is synchronizing the files", func() {
	...
	for _, podman := range []bool{false, true} {
		podman := podman
		When("running astra dev and prestart events are defined", helper.LabelPodmanIf(podman, func() {
			BeforeEach(func() {
				helper.CopyExampleDevFile(
			...
			It("should not correctly execute PreStart commands", func() {
				...
				// see https://github\.com/danielpickens/astra/issues/4187 for more info
	...
	for _, podman := range []bool{false, true} {
		podman := podman
		When("running astra dev and run command throws an error", helper.LabelPodmanIf(podman, func() {
			var devSession helper.DevSession
			BeforeEach(func() {
				helper.CopyExampleDevFile(
					filepath.Join("source", "devfiles", "nodejs", "devfile.yaml"),
	...
	for _, podman := range []bool{false, true} {
		for _, noWatch := range []bool{false, true} {
			...
			title := fmt.Sprintf("running astra dev%s and build command throws an error", noWatchFlag)
	...
	for _, podman := range []bool{false, true} {
		...
		When("Create and dev java-springboot component", helper.LabelPodmanIf(podman, func() {
			...
			BeforeEach(func() {
				...
				helper.Cmd("astra", "init", "--name", devfileCmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "springboot", "devfile.yaml")).ShouldPass()
	...
	for _, podman := range []bool{false, true} {
		podman := podman
		When("setting git config and running astra dev", func() {
			remoteURL := "https://github.com/astra-devfiles/nodejs-ex"
			...
			BeforeEach(func() {
				...
				helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
				helper.Cmd("astra", "init", "--name", devfileCmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile.yaml")).ShouldPass()
			})

			It("should create vcs-uri annotation for the deployment when running astra dev",
	...
	for _, podman := range []bool{true, false} {
		...
		for _, devfileHandlerCtx := range []struct {
			...
			devfileHandlerCtx := devfileHandlerCtx
			When("running astra dev with alternative commands - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				...
				When("running astra dev with a build command", func() {
				...
				When("running astra dev with a run command", func() {
	...
	// Tests https://github\.com/danielpickens/astra/issues/3838
	for _, podman := range []bool{true, false} {
		podman := podman
		When("java-springboot application is created and running astra dev", helper.LabelPodmanIf(podman, func() {
			var devSession helper.DevSession
			var component helper.Component
			BeforeEach(func() {
				helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "springboot", "devfile-registry.yaml")).ShouldPass()
				helper.CopyExample(filepath.Join("source", "devfiles", "springboot", "project"), commonVar.Context)
				component = helper.NewComponent(cmpName, "app", labels.ComponentDevMode, commonVar.Project, commonVar.CliRunner)
				var err error
				devSession, err = helper.StartDevMode(helper.DevSessionOpts{
		...
		When("node-js application is created and deployed with devfile schema 2.2.0", helper.LabelPodmanIf(podman, func() {
			...
			ensureResource := func(memorylimit, memoryrequest string) {
				...
				if !podman {
					...
					// Tastra(feloy) are they taken into account?
	...
	for _, podman := range []bool{false, true} {
		podman := podman
		When("creating nodejs component, doing astra dev and run command has dev.astra.push.path attribute", helper.LabelPodmanIf(podman, func() {
			var devSession helper.DevSession
			var devStarted bool
			BeforeEach(func() {
	...
	// cf. https://github\.com/danielpickens/astra/blob/24fd02673d25eb4c7bb166ec3369554a8e64b59c/tests/integration/devfile/cmd_devfile_delete_test.go#L172-L238
	...
	for _, devfileHandlerCtx := range []struct {
		...
		for _, podman := range []bool{true, false} {
			...
			When("a container component defines a Command or Args - "+devfileHandlerCtx.name, helper.LabelPodmanIf(podman, func() {
				...
				It("should run astra dev successfully (#5620)", func() {
					...
					component.Exec("runtime",
						[]string{
							...
							"-c",
							fmt.Sprintf("kill -0 $(cat %s/.astra_cmd_run.pid) 2>/dev/null ; echo -n $?",
	...
	for _, podman := range []bool{true, false} {
		...
		When("a component with multiple endpoints is run", helper.LabelPodmanIf(podman, func() {
			...
			BeforeEach(func() {
				...
				helper.Cmd("astra", "init", "--name", cmpName, "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile-with-multiple-endpoints.yaml")).ShouldPass()
				Expect(helper.VerifyFileExists(".astra/devstate.json")).To(BeFalse())
				var err error
				devSession, err = helper.StartDevMode(helper.DevSessionOpts{
					RunOnPodman: podman,
				})
		...
		When("a component with multiple endpoints is run", helper.LabelPodmanIf(podman, func() {
			...
			It("should create state files containing information, including forwarded ports", func() {
				...
				By("creating a devsate.$PID.json file with same content as devstate.json", func() {
					pidStateFile := fmt.Sprintf(".astra/devstate.%d.json", pid)
		...
		// 1. git ignore should be modified before astra dev
		// 2. should not pass on podman
		When("a devfile with a local parent is used for astra dev and the parent is not synced", helper.LabelPodmanIf(podman, func() {
			var devSession helper.DevSession
			BeforeEach(func() {
				helper.CopyExampleDevFile(
	...
	When("using devfile that contains K8s resource to run it on podman", Label(helper.LabelPodman), func() {
		...
		BeforeEach(func() {
			...
			devSession, err = helper.StartDevMode(
				helper.DevSessionOpts{RunOnPodman: true, EnvVars: []string{"astra_PUSH_IMAGES=false"}},
			)
			Expect(err).ToNot(HaveOccurred())
		})
		AfterEach(func() {
		...
		It("should show warning about being unable to create the resource when running astra dev on podman", func() {
		...
		It("should build the images when running astra dev on podman", func() {
			// we do not test push because then it becomes complex to setup image registry credentials to pull the image
			// all pods created by astra have a `PullAlways` image policy.
	...
	for _, podman := range []bool{true, false} {
		podman := podman
		When("a hotReload capable Run command is used with astra dev", helper.LabelPodmanIf(podman, func() {
			var devSession helper.DevSession
		...
		When("hotReload capable Build and Run commands are used with astra dev", helper.LabelPodmanIf(podman, func() {
			var devSession helper.DevSession
	...
	for _, podman := range []bool{true, false} {
		...
		Describe("Devfile with no metadata.name", helper.LabelPodmanIf(podman, func() {
			...
			When("running astra dev against a component with no source code", func() {
				var devSession helper.DevSession
				BeforeEach(func() {
					var err error
					devSession, err = helper.StartDevMode(helper.DevSessionOpts{
				...
				It("should use the directory as component name", func() {
					...
					component.Exec("runtime",
						[]string{
							remotecmd.ShellExecutable,
							"-c",
							fmt.Sprintf("cat %s/.astra_cmd_devrun.pid", strings.TrimSuffix(storage.SharedDataMountPath, "/")),
						},
						pointer.Bool(true),
					)
				})
		...
		for _, podman := range []bool{true, false} {
			...
			When(t.whenTitle, helper.LabelPodmanIf(podman, func() {
				...
				if !podman { // not implement for podman
					When("astra deploy is executed", func() {
						BeforeEach(func() {

							helper.Cmd("astra", "deploy").ShouldPass()
						})

						AfterEach(func() {
							helper.Cmd("astra", "delete", "component", "--force")
						})
	...
	for _, ctx := range []struct {
		devfile   string
		checkFunc func(pastraut *corev1.Pod)
		podman    bool
	}{
		{
			devfile: "devfile-pod-container-overrides.yaml",
			checkFunc: func(pastraut *corev1.Pod) {
				Expect(pastraut.Spec.Containers[0].Resources.Limits.Memory().String()).To(ContainSubstring("512Mi"))
				Expect(pastraut.Spec.Containers[0].Resources.Limits.Cpu().String()).To(ContainSubstring("250m"))
				Expect(pastraut.Spec.ServiceAccountName).To(ContainSubstring("new-service-account"))
			},
			podman: false,
		},
		{
			...
			checkFunc: func(pastraut *corev1.Pod) {
				Expect(pastraut.Spec.Containers[0].SecurityContext.RunAsUser).To(Equal(pointer.Int64(1001)))
				Expect(pastraut.Spec.Containers[0].SecurityContext.RunAsGroup).To(Equal(pointer.Int64(1001)))
			},
			podman: true,
		},
	} {
		...
		Context("Devfile contains pod-overrides and container-overrides attributes", helper.LabelPodmanIf(ctx.podman, func() {
			...
			It("should override the content in the pod it creates for the component on the cluster", func() {
				err := helper.RunDevMode(helper.DevSessionOpts{
					...
					component := helper.NewComponent(cmpName, "app", labels.ComponentDevMode, commonVar.Project, commonVar.CliRunner)
					pastraut := component.GetPodDef()
					ctx.checkFunc(pastraut)
				})
				Expect(err).To(BeNil())
			})
		}))
	...
	Context("astra dev on podman when podman in unavailable", Label(helper.LabelPodman), func() {
		BeforeEach(func() {
			helper.CopyExampleDevFile(filepath.Join("source", "devfiles", "nodejs", "devfile.yaml"),
				filepath.Join(commonVar.Context, "devfile.yaml"),
				cmpName)
		})
		It("should fail to run astra dev", func() {
			errOut := helper.Cmd("astra", "dev", "--platform", "podman").WithEnv("PODMAN_CMD=echo").ShouldFail().Err()
	...
	Context("astra dev on podman with a devfile bound to fail", Label(helper.LabelPodman), func() {
		BeforeEach(func() {
			helper.CopyExample(filepath.Join("source", "devfiles", "nodejs", "project"), commonVar.Context)
			helper.CopyExampleDevFile(filepath.Join("source", "devfiles", "nodejs", "devfile.yaml"),
				filepath.Join(commonVar.Context, "devfile.yaml"),
	...
	When("running applications listening on the container loopback interface", func() {
		...
		for _, plt := range []string{"", "cluster"} {
			...
			It("should error out if using --ignore-localhost on any platform other than Podman", func() {
				...
				stderr := helper.Cmd("astra", args...).ShouldFail().Err()
		...
		Context("running on Podman", Label(helper.LabelPodman), func() {
			...
			It("should error out if using both --ignore-localhost and --forward-localhost", func() {
				stderr := helper.Cmd("astra", "dev", "--random-ports", "--platform", "podman", "--ignore-localhost", "--forward-localhost").
					ShouldFail().
					Err()
			...
			When("ignoring localhost", func() {
				...
				It("should port-forward successfully", func() {
					...
					By("not succeeding to reach the local port for the loopback interface", func() {
						...
						// See https://github\.com/danielpickens/astra/issues/6510 and https://github.com/containers/podman/issues/17353
				...
				It("should port-forward successfully", func() {
					...
					By("creating a pod with a two-containers pod because --forward-localhost is true", func() {
						...
						for _, container := range podDef.Spec.Containers {
							if container.Name == fmt.Sprintf("%s-app-astra-helper-port-forwarding", cmpName) {
								pfHelperContainer = container
								found = true
								break
							}
						}
						Expect(found).Should(BeTrue(), fmt.Sprintf("Could not find container 'astra-helper-port-forwarding' in pod spec: %v", podDef))
		...
		Context("image names as selectors", helper.LabelPodmanIf(podman, func() {
			...
			When("starting with a Devfile with relative and absolute image names and Kubernetes resources", func() {
				...
				When("adding a local registry for images", func() {
					...
					BeforeEach(func() {
						helper.Cmd("astra", "preference", "set", "ImageRegistry", imageRegistry, "--force").ShouldPass()
					})

					AfterEach(func() {
						helper.Cmd("astra", "preference", "unset", "ImageRegistry", "--force").ShouldPass()
					})
					...
					When("running astra dev", func() {
						...
						It("should treat relative image names as selectors", func() {
							...
							By("building and optionally pushing absolute image components with no replacement", func() {
								for _, img := range []string{"ttl.sh/astra-dev-node:1h", "ttl.sh/nodejs-devtools2:1h"} {
									Expect(imagesProcessed).Should(ContainElement(img))
								}
							})

							if !podman {
								// On Podman, `astra dev` just warns if there are any Kubernetes/OpenShift components at the moment. But this is already tested elsewhere
								...
								By("not replacing non-matching or absolute image names in core Kubernetes resources", func() {
									...
									for resType, data := range k8sResourcesDeployed {
										...
										Expect(data.initContainers[initCont1]).Should(
											Equal("ttl.sh/astra-dev-node:1h"),
											func() string {
												return fmt.Sprintf(
