...
import (
	...
	astralog "github\.com/danielpickens/astra/pkg/log"
	"github\.com/danielpickens/astra/tests/helper"
)

var _ = Describe("astra init interactive command tests", Label(helper.LabelSkipOnOpenShift), func() {
	...
	for _, label := range []string{
		...
		var _ = Context("label "+label, Label(label), func() {
			...
			It("should print automation command with proper values", func() {
				...
				Expect(err).To(BeNil())
				Expect(output).To(ContainSubstring("astra init --name %s --devfile %s --devfile-registry DefaultDevfileRegistry --devfile-version %s --starter %s", componentName, devfileName, devfileVersion, starter))
				Expect(output).To(ContainSubstring("Your new component 'my-go-app' is ready in the current directory"))
				Expect(helper.ListFilesInDir(commonVar.Context)).To(ContainElements("devfile.yaml"))
			...
			Describe("displaying welcoming messages", func() {
				...
				testRunner := func(language string, welcomingMsgs []string, tester helper.Tester) (string, error) {
					...
					return helper.RunInteractive(command,
						...
						// Using envvars here (and not via the -v flag), because of https://github\.com/danielpickens/astra/issues/5513
						[]string{"astra_LOG_LEVEL=0"},
				...
				When("directory is empty", func() {
					...
					It("should display appropriate welcoming messages", func() {
						...
						astraVersion, gitCommit := helper.GetastraVersion()
						welcomingMsgs := strings.Split(
							astralog.StitleWithVersion(messages.InitializingNewComponent,
								messages.NoSourceCodeDetected,
								fmt.Sprintf("astra version: %s (%s)", astraVersion, gitCommit)),
							"\n")
				...
				When("directory is not empty", func() {
					...
					It("should display appropriate welcoming messages", func() {
						...
						output, err := testRunner(language, welcomingMsgs, func(ctx helper.InteractiveContext) {
							helper.ExpectString(ctx, "Based on the files in the current directory astra detected")
						...
						assertBehavior(strings.ToLower(language), output, err, welcomingMsgs, func() {
							...
							Expect(helper.ListFilesInDir(commonVar.Context)).To(
								SatisfyAll(
									HaveLen(4),
									ContainElements("devfile.yaml", "requirements.txt", "wsgi.py", util.DotastraDirectory)))
						})
					})
				})
				...
				When("alizer detection of javascript name", func() {
					...
					It("should display node-echo name", func() {
						...
						output, err := helper.RunInteractive([]string{"astra", "init"}, nil, func(ctx helper.InteractiveContext) {
							helper.ExpectString(ctx, "Based on the files in the current directory astra detected")
					...
					It("should ask to re-enter the component name if invalid value is passed by the user", func() {
						...
						_, err := helper.RunInteractive([]string{"astra", "init"}, nil, func(ctx helper.InteractiveContext) {
							helper.ExpectString(ctx, "Based on the files in the current directory astra detected")
			...
			It("should download correct devfile from the first in-cluster registry", func() {
				...
				By("displaying automation command with the in-cluster registry", func() {
					Expect(output).Should(
						ContainSubstring(
							"astra init --name my-java-maven-app --devfile java-maven --devfile-registry %s --devfile-version 1.2.0 --starter springbootproject",
							devfileRegistryName,
						),
						output)
				})
