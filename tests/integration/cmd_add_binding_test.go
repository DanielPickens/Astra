...
var _ = Describe("astra add binding command tests", Label(helper.LabelServiceBinding), Label(helper.LabelSkipOnOpenShift), func() {
	// Tastra: Add integration tests for the following cases when SBO is not installed
	// * servicebinding with envvars does not send `astra dev` in an infinite loop
	// * `astra dev` deletes service binding secrets for SB that is not present locally
...
`))
	...
	When("binding to a service in a different namespace", func() {
		...
		It("should error out if service is not found in the namespace selected", func() {
			stderr := helper.Cmd("astra", "add", "binding",
				"--name", "aname",
	...
	When("the component is bootstrapped", func() {
		BeforeEach(func() {
			helper.Cmd("astra", "init", "--name", "mynode", "--devfile-path", helper.GetExamplePath("source", "devfiles", "nodejs", "devfile.yaml"), "--starter", "nodejs-starter").ShouldPass()
		})

		It("should fail using the --workload parameter", func() {
			stderr := helper.Cmd("astra", "add", "binding", "--name", "aname", "--service", "cluster-sample", "--workload", "app/Deployment.apps").ShouldFail().Err()
		...
		for _, ctx := range []struct {
		...
		}{
			{
				...
				beforeEachFunc: func(bindingName string) string {
					helper.Cmd("astra", "add", "binding", "--name", bindingName, "--service", "cluster-sample").ShouldPass()
    ...
    version: v1`, bindingName)))
			...
			},
			{
				...
				beforeEachFunc: func(bindingName string) string {
					...
					helper.Cmd("astra", "add", "binding", "--name", bindingName,
    ...
    version: v1`, bindingName, ns)))
			...
			When("adding a binding ("+ctx.name+")", func() {
				...
				When("astra dev is run", func() {
					...
					When("astra dev command is stopped", func() {
						BeforeEach(func() {
							devSession.Stop()
							devSession.WaitEnd()
						})
		...
		When("no bindable instance is present on the cluster", func() {
			...
			It("should fail to add binding with no bindable instance found error message", func() {
				errOut := helper.Cmd("astra", "add", "binding", "--name", "my-binding", "--service", "cluster-sample").ShouldFail().Err()
