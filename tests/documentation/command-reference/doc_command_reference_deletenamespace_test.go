...
var _ = Describe("doc command reference astra delete namespace", func() {
	...
	Context("To delete a namespace resource", func() {
		...
		BeforeEach(func() {
			helper.Cmd("astra", "create", "namespace", ns).ShouldPass()
		})
		...
		It("Deletes a namespace resource for a kubernetes cluster", func() {
			args := []string{"astra", "delete", "namespace", ns}
			...
			got = strings.ReplaceAll(got, ns, "astra-dev")
		...
		It("Deletes a project resource for a openshift cluster", func() {
			args := []string{"astra", "delete", "project", ns}
			out, err := helper.RunInteractive(args, []string{"astra_LOG_LEVEL=0"}, func(ctx helper.InteractiveContext) {
