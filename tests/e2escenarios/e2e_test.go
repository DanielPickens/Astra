...
var _ = Describe("E2E Test", func() {
	...
	Context("starting with empty Directory", func() {
		It("should verify developer workflow from empty Directory", func() {
			...
			// "execute astra dev and add changes to application"
			...
			// "exit dev mode and run astra deploy"
			devSession.Stop()
			devSession.WaitEnd()
			...
			// "run astra delete and check if the component is deleted"
			command = []string{"astra", "delete", "component"}
			_, err = helper.RunInteractive(command, nil, func(ctx helper.InteractiveContext) {
	...
	Context("starting with non-empty Directory add Binding", Label(helper.LabelServiceBinding), func() {
		...
		It("should verify developer workflow of using binding as env in innerloop", func() {
			...
			helper.Cmd("astra", "add", "binding", "--name", bindingName, "--service", "cluster-example-initdb", "--bind-as-files=false").ShouldPass()
			...
			// check astra describe to check for env
			stdout := helper.Cmd("astra", "describe", "binding").ShouldPass().Out()
			helper.MatchAllInOutput(stdout, []string{"Available binding information:", "CLUSTER_HOST", "CLUSTER_PASSWORD", "CLUSTER_USERNAME"})

			// "running astra list"
			stdout = helper.Cmd("astra", "list").ShouldPass().Out()
			...
			err = devSession.WaitSync()
			Expect(err).To(BeNil())
			Eventually(func() string { return helper.Cmd("astra", "describe", "binding").ShouldRun().Out() }).
