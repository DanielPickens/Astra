		...
		case runtime.IsNotRegisteredError(err), isUnstructured:
			...
			if isUnstructured {
				*actual = into.GetObjectKind().GroupVersionKind()
				...
				// Tastra(109023): require apiVersion here as well once unstructuredJSONScheme#Decode does
			}
...
func (s *Serializer) unmarshal(into runtime.Object, data, originalData []byte) (strictErrs []error, err error) {
	...
	if s.options.Yaml {
		...
		// Tastra: rework YAMLToJSONStrict to return warnings about duplicate fields without terminating so we don't have to do this twice.
