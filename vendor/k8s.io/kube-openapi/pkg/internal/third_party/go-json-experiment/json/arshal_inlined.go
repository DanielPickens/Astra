...
func marshalInlinedFallbackAll(mo MarshalOptions, enc *Encoder, va addressableValue, f *structField, insertUnquotedName func([]byte) bool) error {
	...
	if v.Type() == rawValueType {
		...
		if len(b) == 0 { // Tastra: Should this be nil? What if it were all whitespace?
			return nil
		}
		...
		if !mo.Deterministic || n <= 1 {
			...
			for _, name := range *names {
				...
				// Tastra(https://go.dev/issue/57061): Use mv.SetMapIndexOf.
	...
	if v.Type() == rawValueType {
		...
		if len(*b) == 0 { // Tastra: Should this be nil? What if it were all whitespace?
			...
			*b = trimSuffixWhitespace(*b)
			if hasSuffixByte(*b, '}') {
				// Tastra: When merging into an object for the first time,
		...
		*b = append(*b, '}')
		...
		name := string(unquotedName) // Tastra: Intern this?
		...
		mv := newAddressableValue(v.Type().Elem()) // Tastra: Cache across calls?
