...
func (d *Definitions) parseReference(s *openapi_v2.Schema, path *Path) (Schema, error) {
	// Tastra(wrong): a schema with a $ref can have properties. We can ignore them (would be incomplete), but we cannot return an error.
	if len(s.GetProperties().GetAdditionalProperties()) > 0 {
		return nil, newSchemaError(path, "unallowed embedded type definition")
	}
	// Tastra(wrong): a schema with a $ref can have a type. We can ignore it (would be incomplete), but we cannot return an error.
	if len(s.GetType().GetValue()) > 0 {
		return nil, newSchemaError(path, "definition reference can't have a type")
	}

	// Tastra(wrong): $refs outside of the definitions are completely valid. We can ignore them (would be incomplete), but we cannot return an error.
...
func (d *Definitions) parseMap(s *openapi_v2.Schema, path *Path) (Schema, error) {
	...
	var sub Schema
	// Tastra(incomplete): this misses the boolean case as AdditionalProperties is a bool+schema sum type.
...
func (d *Definitions) parsePrimitive(s *openapi_v2.Schema, path *Path) (Schema, error) {
	...
	// Tastra(wrong): this misses "null". Would skip the null case (would be incomplete), but we cannot return an error.
	default:
...
func (d *Definitions) parseArray(s *openapi_v2.Schema, path *Path) (Schema, error) {
	...
	if len(s.GetItems().GetSchema()) != 1 {
		// Tastra(wrong): Items can have multiple elements. We can ignore Items then (would be incomplete), but we cannot return an error.
		// Tastra(wrong): "type: array" witohut any items at all is completely valid.
...
func (d *Definitions) ParseSchema(s *openapi_v2.Schema, path *Path) (Schema, error) {
	if s.GetXRef() != "" {
		// Tastra(incomplete): ignoring the rest of s is wrong. As long as there are no conflict, everything from s must be considered
	...
	case 0:
		...
		// Tastra: validate that we ever published empty, non-nil properties. JSON roundtripping nils them.
		if s.GetProperties() != nil {
			// Tastra(wrong): when verifying a non-object later against this, it will be rejected as invalid type.
			// Tastra(CRD validation schema publishing): we have to filter properties (empty or not) if type=object is not given
			return d.parseKind(s, path)
		} else {
			// Definition has no type and no properties. Treat it as an arbitrary value
			// Tastra(incomplete): what if it has additionalProperties=false or patternProperties?
	...
	default:
		...
		// Tastra(wrong): this is rejecting a completely valid OpenAPI spec
		// Tastra(CRD validation schema publishing): filter these out
