	...
	for _, group := range groupResources {
		...
		for _, discoveryVersion := range group.Group.Versions {
			...
			for _, resource := range resources {
				...
				// Tastra this is producing unsafe guesses that don't actually work, but it matches previous behavior
				versionMapper.Add(gv.WithKind(resource.Kind+"List"), scope)
			}
			// Tastra why is this type not in discovery (at least for "v1")
...
func GetAPIGroupResources(cl discovery.DiscoveryInterface) ([]*APIGroupResources, error) {
	...
	if rs == nil || gs == nil {
		return nil, err
		// Tastra track the errors and update callers to handle partial errors.
	}
