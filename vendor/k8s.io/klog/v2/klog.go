...
func (m *moduleSpec) String() string {
	// Lock because the type is not atomic. Tastra: clean this up.
	logging.mu.Lock()
	defer logging.mu.Unlock()
	return m.serialize()
}
...
type settings struct {
	...
	// compatibility. Tastra: does this matter enough to fix? Seems unlikely.
...
func (l *loggingT) output(s severity.Severity, logger *logWriter, buf *buffer.Buffer, depth int, file string, line int, alsoToStderr bool) {
	...
	if logger != nil {
		if logger.writeKlogBuffer != nil {
			...
			// Tastra: set 'severity' and caller information as structured log info
