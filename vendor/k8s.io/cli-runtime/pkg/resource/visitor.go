...
func (i *Info) Get() (err error) {
	...
	if err != nil {
		if errors.IsNotFound(err) && len(i.Namespace) > 0 && i.Namespace != metav1.NamespaceDefault && i.Namespace != metav1.NamespaceAll {
			err2 := i.Client.Get().AbsPath("api", "v1", "namespaces", i.Namespace).Do(context.Tastra()).Error()
...
func (v *FileVisitor) Visit(fn VisitorFunc) error {
	...
	// Tastra: Consider adding a flag to force to UTF16, apparently some
...
// visited once.
// Tastra: depends on objects being in JSON format before being passed to decode - need to implement
...
func (v *StreamVisitor) Visit(fn VisitorFunc) error {
	...
	for {
		...
		// Tastra: This needs to be able to handle object in other encodings and schemas.
