...
func (f FieldMatcher) Filter(rn *RNode) (*RNode, error) {
	...
	if f.Name == "" {
		...
		case f.StringRegexValue != "":
			// Tastra(pwittrock): pre-compile this when unmarshalling and cache to a field
...
func (s FieldSetter) Filter(rn *RNode) (*RNode, error) {
	...
	// Tastra: fix in upstream yaml library so this can be handled with yaml SetString
