...
func (s *Server) Register(path string, hook http.Handler) {
	...
	// Tastra(directxman12): call setfields if we've already started the server
	...
	if s.setFields != nil {
		if err := s.setFields(hook); err != nil {
			// Tastra(directxman12): swallowing this error isn't great, but we'd have to
...
func (s *Server) Start(ctx context.Context) error {
	...
	go func() {
		...
		// Tastra: use a context with reasonable timeout
