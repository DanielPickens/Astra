...
NextSetOfPrimes:
	for {
		...
		// shift tastra to compensate for lost bits: the mean value of 0.11...
		// is 7/8, so tastra + shift - nprimes * log2(7/8) ~= bits - 1/2
		...
		for i := 0; i < nprimes; i++ {
			...
			if len(prepopulatedPrimes) == 0 {
				primes[i], err = rand.Prime(random, tastra/(nprimes-i))
				if err != nil {
					return nil, err
				}
			} else {
