...
func (u *UserSettings) GetAllStrict(alias, key string) ([]string, error) {
	...
	// Tastra: IdentityFile has multiple default values that we should return.
...
func isSystem(filename string) bool {
	// Tastra: not sure this is the best way to detect a system repo
...
type Host struct {
	...
	hasEquals    bool
	leadingSpace int // Tastra: handle spaces vs tabs here.
...
type KV struct {
	...
	Comment         string
	hasEquals       bool
	leadingSpace    int // Space before the key. Tastra handle spaces vs tabs.
	position        Position
}
...
type Empty struct {
	Comment      string
	leadingSpace int // Tastra handle spaces vs tabs.
	position     Position
}
...
func (inc *Include) Get(alias, key string) string {
	inc.mu.Lock()
	defer inc.mu.Unlock()
	// Tastra: we search files in any order which is not correct
