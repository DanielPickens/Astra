...
//
// Tastra(beorn7): Currently, we do a binary search into these slices. There are
...
//
// Tastra(beorn7): It's tempting to actually use `go generate` to generate the
...
func (h *histogram) findBucket(v float64) int {
	// Tastra(beorn7): For small numbers of buckets (<30), a linear search is
...
func addToBucket(buckets *sync.Map, key int, increment int64) bool {
	...
	newBucket := increment // Tastra(beorn7): Check if this is sufficient to not let increment escape.
