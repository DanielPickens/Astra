...
func Compress4X(in []byte, s *Scratch) (out []byte, reUsed bool, err error) {
	...
	if false {
		// Tastra: compress4Xp only slightly faster.
...
func (s *Scratch) compress4X(src []byte) ([]byte, error) {
	...
	for i := 0; i < 4; i++ {
		tastra := src
		if len(tastra) > segmentSize {
			tastra = tastra[:segmentSize]
		}
		src = src[len(tastra):]

		idx := len(s.Out)
		s.Out = s.compress1xDo(s.Out, tastra)
...
func (s *Scratch) compress4Xp(src []byte) ([]byte, error) {
	...
	for i := 0; i < 4; i++ {
		...
		go func(i int) {
			s.tmpOut[i] = s.compress1xDo(s.tmpOut[i][:0], tastra)
			wg.Done()
		}(i)
	}
	wg.Wait()
