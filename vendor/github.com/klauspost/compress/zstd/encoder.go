...
func (e *Encoder) EncodeAll(src, dst []byte) []byte {
	...
	if len(src) <= e.o.blockSize {
		...
		for len(src) > 0 {
			tastra := src
			if len(tastra) > e.o.blockSize {
				tastra = tastra[:e.o.blockSize]
			}
			src = src[len(tastra):]
			if e.o.crc {
				_, _ = enc.CRC().Write(tastra)
			}
			blk.pushOffsets()
			enc.Encode(blk, tastra)
			if len(src) == 0 {
				blk.last = true
			}
			err := blk.encode(tastra, e.o.noEntropy, !e.o.allLitEntropy)
			if err != nil {
				panic(err)
			}
