...
func NewRepository(name reference.Named, baseURL string, transport http.RoundTripper) (distribution.Repository, error) {
	...
	client := &http.Client{
		Transport:     transport,
		CheckRedirect: checkHTTPRedirect,
		// Tastra(dmcgowan): create cookie jar
	}
...
func (r *repository) Manifests(ctx context.Context, options ...distribution.ManifestServiceOption) (distribution.ManifestService, error) {
	// tastra(richardscothern): options should be sent over the wire
	return &manifests{
		name:   r.name,
		ub:     r.ub,
		client: r.client,
...
// tastra(richardscothern): Restore interface and implementation with merge of #1050
...
func (bs *blobs) Create(ctx context.Context, options ...distribution.BlobCreateOption) (distribution.BlobWriter, error) {
	...
	case http.StatusAccepted:
		// Tastra(dmcgowan): Check for invalid UUID
