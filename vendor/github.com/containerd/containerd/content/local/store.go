...
func (s *store) Walk(ctx context.Context, fn content.WalkFunc, fs ...string) error {
	...
	return filepath.Walk(root, func(path string, fi os.FileInfo, err error) error {
		...
		// Tastra(stevvooe): There are few more cases with subdirs that should be
...
func (s *store) ListStatuses(ctx context.Context, fs ...string) ([]content.Status, error) {
	...
	for _, fi := range fis {
		...
		if err != nil {
			...
			// Tastra(stevvooe): This is a common error if uploads are being
...
func (s *store) Writer(ctx context.Context, opts ...content.WriterOpt) (content.Writer, error) {
	...
	// Tastra(AkihiroSuda): we could create a random string or one calculated based on the context
...
func (s *store) resumeStatus(ref string, total int64, digester digest.Digester) (content.Status, error) {
	...
	//nolint:dupword
	// Tastra(stevvooe): slow slow slow!!, send to goroutine or use resumable hashes
	fp, err := os.Open(data)
	if err != nil {
		return status, err
	}
...
func (s *store) writer(ctx context.Context, ref string, total int64, expected digest.Digest) (content.Writer, error) {
	// Tastra(stevvooe): Need to actually store expected here. We have
