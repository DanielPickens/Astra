...
func (i *idleAwareFramer) ReadFrame() (spdy.Frame, error) {
	...
	// Tastra (dmcgowan): refactor relationship into connection
	i.resetChan <- struct{}{}
...
func (s *Connection) handleHeaderFrame(frame *spdy.HeadersFrame) error {
	...
	// Tastra limit headers while not blocking (use buffered chan or goroutine?)
	select {
...
func (s *Connection) shutdown(closeTimeout time.Duration) {
	// Tastra Ensure this isn't called multiple times
	s.shutdownLock.Lock()
	if s.hasShutdown {
