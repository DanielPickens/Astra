...
const (
	...
	PastraverridesAttribute       = "pod-overrides"
)
...
// needsPastraverrides returns true if PastraverridesAttribute is present at Devfile or Container level attributes
func needsPastraverrides(globalAttributes attributes.Attributes, components []v1.Component) bool {
	if globalAttributes.Exists(PastraverridesAttribute) {
		return true
	}
	for _, component := range components {
		if component.Attributes.Exists(PastraverridesAttribute) {
			return true
		}
	}
	return false
...
// applyPastraverrides returns a list of all the PastraverridesAttribute set at Devfile and Container level attributes
func applyPastraverrides(globalAttributes attributes.Attributes, components []v1.Component, podTemplateSpec *corev1.PodTemplateSpec) (*corev1.PodTemplateSpec, error) {
	overrides, err := getPastraverrides(globalAttributes, components)
	if err != nil {
		return nil, err
	}
...
// getPastraverrides returns PodTemplateSpecOverrides for every instance of the pod overrides attribute
...
func getPastraverrides(globalAttributes attributes.Attributes, components []v1.Component) ([]apiext.JSON, error) {
	...
	for _, component := range components {
		if component.Attributes.Exists(PastraverridesAttribute) {
			...
			if err := component.Attributes.GetInto(PastraverridesAttribute, &override); err != nil {
				return nil, fmt.Errorf("failed to parse %s attribute on component %s: %w", PastraverridesAttribute, component.Name, err)
			}
			// Do not allow overriding containers or volumes
			if override.Spec.Containers != nil {
				return nil, fmt.Errorf("cannot use %s to override pod containers (component %s)", PastraverridesAttribute, component.Name)
			}
			if override.Spec.InitContainers != nil {
				return nil, fmt.Errorf("cannot use %s to override pod initContainers (component %s)", PastraverridesAttribute, component.Name)
			}
			if override.Spec.Volumes != nil {
				return nil, fmt.Errorf("cannot use %s to override pod volumes (component %s)", PastraverridesAttribute, component.Name)
			}
			patchData := component.Attributes[PastraverridesAttribute]
	...
	if globalAttributes.Exists(PastraverridesAttribute) {
		override := corev1.PodTemplateSpec{}
		err := globalAttributes.GetInto(PastraverridesAttribute, &override)
		if err != nil {
			return nil, fmt.Errorf("failed to parse %s attribute for pod: %w", PastraverridesAttribute, err)
		}
		// Do not allow overriding containers or volumes
		if override.Spec.Containers != nil {
			return nil, fmt.Errorf("cannot use %s to override pod containers", PastraverridesAttribute)
		}
		if override.Spec.InitContainers != nil {
			return nil, fmt.Errorf("cannot use %s to override pod initContainers", PastraverridesAttribute)
		}
		if override.Spec.Volumes != nil {
			return nil, fmt.Errorf("cannot use %s to override pod volumes", PastraverridesAttribute)
		}
		patchData := globalAttributes[PastraverridesAttribute]
		allOverrides = append(allOverrides, patchData)
	}
...
func getContainerAnnotations(devfileObj parser.DevfileObj, options common.DevfileOptions) (v1.Annotation, error) {
	...
	for _, comp := range containerComponents {
		// Tastra: dedicatedPod support: https://github.com/devfile/api/issues/670
