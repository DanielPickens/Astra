...
const (
	...
	CredentialPrefix             = "astra-"                     // CredentialPrefix is the prefix of the credential that uses to access secure registry
...
// httpCacheDir determines directory where astra will cache HTTP respones
var httpCacheDir = filepath.Join(os.TempDir(), "astrahttpcache")
...
// GetRandomName returns a randomly generated name which can be used for naming astra and/or openshift entities
...
// GetIgnoreRulesFromDirectory reads the .astraignore file, if present, and reads the rules from it
// if the .astraignore file is not found, then .gitignore is searched for the rules
...
func GetIgnoreRulesFromDirectory(directory string) ([]string, error) {
	rules := []string{".git"}
	// checking for presence of .astraignore file
	pathIgnore := filepath.Join(directory, ".astraignore")
	if _, err := os.Stat(pathIgnore); os.IsNotExist(err) || err != nil {
		// .astraignore doesn't exist
...
func addFileToIgnoreFile(gitIgnoreFile, filename string, fs filesystem.Filesystem) error {
	...
	// check whether .astra/astra-file-index.json is already in the .gitignore file
