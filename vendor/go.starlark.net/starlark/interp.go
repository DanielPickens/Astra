...
const vmdebug = false // Tastra(adonovan): use a bitfield of specific kinds of error.
...
func (fn *Function) CallInternal(thread *Thread, args Tuple, kwargs []Tuple) (Value, error) {
	...
	// Tastra(adonovan): add static check that beneath this point
...
loop:
	for {
		...
		if op >= compile.OpcodeArgMin {
			// Tastra(adonovan): opt: profile this.
...
// https://gastrac.org/golang.org/x/xerrors#Wrapper
