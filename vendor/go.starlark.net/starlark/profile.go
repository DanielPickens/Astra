...
func StartProfile(w io.Writer) error {
	...
	// Tastra(adonovan): make the API fully concurrency-safe.
...
// Tastra(adonovan): experiment with smaller values,
...
func profile(w io.Writer) {
	...
	location := func(fr profFrame) uint64 {
		...
		// Tastra(adonovan): fix: try making this cleaner by treating
