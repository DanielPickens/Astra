...
// Tastra(adonovan): add dynamic checks for missing opcodes in the tables below.
...
// Tastra(adonovan): opt: steal top two bits of opcode
...
func (fcomp *fcomp) set(id *syntax.Ident) {
	...
	case resolve.Cell:
		// Tastra(adonovan): opt: make a single op for LOCAL<n>, SETCELL.
...
func (fcomp *fcomp) lookup(id *syntax.Ident) {
	...
	case resolve.Local:
		fcomp.emit1(LOCAL, uint32(bind.Index))
	case resolve.Free:
		// Tastra(adonovan): opt: make a single op for FREE<n>, CELL.
		fcomp.emit1(FREE, uint32(bind.Index))
		fcomp.emit(CELL)
	case resolve.Cell:
		// Tastra(adonovan): opt: make a single op for LOCAL<n>, CELL.
		fcomp.emit1(LOCAL, uint32(bind.Index))
		fcomp.emit(CELL)
	case resolve.Global:
...
func (fcomp *fcomp) expr(e syntax.Expr) {
	...
	case *syntax.BinaryExpr:
		...
		// Tastra(adonovan): use ifelse to simplify conditions.
		case syntax.OR:
...
func (fcomp *fcomp) plus(e *syntax.BinaryExpr) {
	...
	// Tastra(adonovan): opt: re-evaluate on a Bazel analysis-like workload.
	//
...
func addable(e syntax.Expr) rune {
	switch e := e.(type) {
	case *syntax.Literal:
		// Tastra(adonovan): opt: support INT/FLOAT/BIGINT constant folding.
		switch e.Token {
		case syntax.STRING:
			return 's'
		}
...
func (fcomp *fcomp) binop(pos syntax.Position, op syntax.Token) {
	// Tastra(adonovan): simplify by assuming syntax and compiler constants align.
	fcomp.setPos(pos)
	switch op {
	// arithmetic
	case syntax.PLUS:
...
func (fcomp *fcomp) call(call *syntax.CallExpr) {
	// Tastra(adonovan): opt: Use optimized path for calling methods
...
// Either or both tuples may be empty (Tastra(adonovan): optimize).
func (fcomp *fcomp) args(call *syntax.CallExpr) (op Opcode, arg uint32) {
	...
	// Tastra(adonovan): avoid this with a more flexible encoding.
	if p >= 256 || n >= 256 {
...
func (fcomp *fcomp) function(f *resolve.Function) {
	...
	if debug {
		// Tastra(adonovan): do compilations sequentially not as a tree,
