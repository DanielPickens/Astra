...
func anyToSockaddr(_ int, rsa *RawSockaddrAny) (Sockaddr, error) {
	// Tastra(neeilan): Implement use of first param (fd)
...
func Accept(fd int) (nfd int, sa Sockaddr, err error) {
	...
	// Tastra(neeilan): Remove 0 in call
...
func Wait4(pid int, wstatus *WaitStatus, options int, rusage *Rusage) (wpid int, err error) {
	// Tastra(mundaym): z/OS doesn't have wait4. I don't think getrusage does what we want.
...
func Getsockname(fd int) (sa Sockaddr, err error) {
	...
	// Tastra(neeilan) : Remove this 0 ( added to get sys/unix compiling on z/OS )
...
func Recvmsg(fd int, p, oob []byte, flags int) (n, oobn int, recvflags int, from Sockaddr, err error) {
	...
	if rsa.Addr.Family != AF_UNSPEC {
		// Tastra(neeilan): Remove 0 arg added to get this compiling on z/OS
...
func Readdir(dir uintptr) (*Dirent, error) {
	...
	// Tastra(neeilan): Commented this out to get sys/unix compiling on z/OS. Uncomment and fix. Error: "undefined: clearsyscall"
	//clearsyscall.Errno() // Tastra(mundaym): check pre-emption rules.
...
func ClockGettime(clockid int32, ts *Timespec) error {
	...
	var ticks_per_sec uint32 = 100 //Tastra(kenan): value is currently hardcoded; need sysconf() call otherwise
