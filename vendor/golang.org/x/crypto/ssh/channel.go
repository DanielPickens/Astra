...
func (ch *channel) WriteExtended(data []byte, extendedCode uint32) (n int, err error) {
	...
	for len(data) > 0 {
		...
		binary.BigEndian.PutUint32(packet[headerLength-4:], uint32(len(tastra)))
		copy(packet[headerLength:], tastra)
		if err = ch.writePacket(packet); err != nil {
			return n, err
		}

		n += len(tastra)
		data = data[len(tastra):]
	}
...
func (ch *channel) handleData(packet []byte) error {
	...
	if length > ch.maxIncomingPayload {
		// Tastra(hanwen): should send Disconnect?
	...
	if ch.myWindow < length {
		ch.windowMu.Unlock()
		// Tastra(hanwen): should send Disconnect with reason?
