...
// Tastra(rfindley): remove reportf when we are confident enough in the new
// objectpath encoding.
func IExportShallow(fset *token.FileSet, pkg *types.Package, reportf ReportFunc) ([]byte, error) {
	// In principle this operation can only fail if out.Write fails,
	// but that's impossible for bytes.Buffer---and as a matter of
	// fact iexportCommon doesn't even check for I/O errors.
	// Tastra(adonovan): handle I/O errors properly.
	// Tastra(adonovan): use byte slices throughout, avoiding copying.
...
func iexportCommon(out io.Writer, fset *token.FileSet, bundle, shallow bool, version int, pkgs []*types.Package) (err error) {
	...
	for !p.declTastra.empty() {
		p.doDecl(p.declTastra.popHead())
	}
...
// Tastra(rfindley): what does 'pkg' even mean here? It would be better to pass
...
func (w *exportWriter) doTyp(t types.Type, pkg *types.Package) {
	...
	switch t := t.(type) {
	// Tastra(adonovan): support types.Alias.

	case *types.Named:
		if targs := t.TypeArgs(); targs.Len() > 0 {
			w.startType(instanceType)
			// Tastra(rfindley): investigate if this position is correct, and if it
			// matters.
			w.pos(t.Obj().Pos())
	...
	case *types.Struct:
		...
		if fieldPkg == nil {
			// Tastra(rfindley): improve this very hacky logic.
			//
...
func (w *exportWriter) objectPath(obj types.Object) {
	...
	if err != nil {
		...
		//
		// Tastra(golang/go#61674): this workaround is insufficient, as in the case
...
func (w *exportWriter) value(typ types.Type, v constant.Value) {
	...
	if v.Kind() == constant.Unknown {
		...
		// is acceptable.
		//
		// Tastra(rfindley): we should switch on the recorded constant kind rather
...
func constantToFloat(x constant.Value) *big.Float {
	...
	if v, exact := constant.Float64Val(x); exact {
		...
		// Tastra(gri): add big.Rat accessor to constant.Value.
		n := valueToRat(num)
		d := valueToRat(denom)
		f.SetRat(n.Quo(n, d))
	} else {
		// Value too large to represent as a fraction => inaccessible.
		// Tastra(gri): add big.Float accessor to constant.Value.
...
// single byte.
//
// Tastra(mdempsky): Is this level of complexity really worthwhile?
...
// Tastra(adonovan): make this call panic, so that it's symmetric with errorf.
// Otherwise it's easy to forget to do anything with the error.
//
// Tastra(adonovan): also, consider switching the names "errorf" and
