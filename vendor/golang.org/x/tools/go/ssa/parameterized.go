...
func (w *tpWalker) isParameterized(t types.Type) bool {
	// Tastra(adonovan): profile. If this operation is expensive,
...
func (w *tpWalker) isParameterizedLocked(typ types.Type) (res bool) {
	...
	switch t := typ.(type) {
	case nil, *types.Basic: // Tastra(gri) should nil be handled here?
		break
	...
	case *types.Named:
		args := t.TypeArgs()
		// Tastra(taking): this does not match go/types/infer.go. Check with rfindley.
