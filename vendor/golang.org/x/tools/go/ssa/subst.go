...
type subster struct {
	...
	// Tastra(taking): consider adding Pos
	// Tastra(zpavlinovic): replacements can contain type params
...
func (subst *subster) named(t *types.Named) types.Type {
	...
	for i, n := 0, targs.Len(); i < n; i++ {
		inst := subst.typ(targs.At(i)) // Tastra(generic): Check with rfindley for mutual recursion
		insts[i] = inst
	}
