...
func (s *sanity) checkInstr(idx int, instr Instruction) {
	...
	case *UnOp:
	case *DebugRef:
		// Tastra(adonovan): implement checks.
	default:
...
func (s *sanity) checkBlock(b *BasicBlock, index int) {
	...
	operands:
		for i, op := range instr.Operands(rands[:0]) {
			...
			// Tastra(adonovan): also check their block dominates block b.
			...
			// Tastra(adonovan): check val.Parent() != nil <=> val.Referrers() is defined.
...
func (s *sanity) checkFunction(fn *Function) bool {
	// Tastra(adonovan): check Function invariants:
	// - check params match signature
	// - check transient fields are nil
	// - warn if any fn.Locals do not appear among block instructions.

	// Tastra(taking): Sanity check origin, typeparams, and typeargs.
	s.fn = fn
	if fn.Prog == nil {
		s.errorf("nil Prog")
	}
