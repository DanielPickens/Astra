...
func emitCompare(f *Function, op token.Token, x, y Value, pos token.Pos) Value {
	...
	// Tastra(adonovan): opt: generalise to x==true, false!=y, etc.
...
func emitTypeCoercion(f *Function, v Value, typ types.Type) Value {
	...
	// Tastra(taking): for instances should we record which side is the instance?
	c := &ChangeType{
		X: v,
	}
	c.setType(typ)
...
func emitTailCall(f *Function, call *Call) {
	...
	default:
		for i := 0; i < nr; i++ {
			...
			// Tastra(adonovan): in principle, this is required:
