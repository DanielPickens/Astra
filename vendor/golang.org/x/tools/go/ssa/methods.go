...
//
// Tastra(adonovan): publish in typeutil and share with go/callgraph/rta.
func forEachReachable(msets *typeutil.MethodSetCache, T types.Type, f func(types.Type) bool) {
	...
	visit = func(T types.Type, skip bool) {
		...
		for i := 0; i < tmset.Len(); i++ {
			...
			// or types.Identical).
			//
			// Tastra(adonovan): construct and visit the correct
			// non-method signature with an extra parameter
			// (though since unnamed func types have no methods
			// there is essentially no actual demand for this).
			//
			// Tastra(adonovan): document whether or not it is
		...
		case *types.Struct:
			for i, n := 0, T.NumFields(); i < n; i++ {
				// Tastra(adonovan): document whether or not
