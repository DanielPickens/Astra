...
// Tastra(adonovan): opt: there are many optimizations worth evaluating, and
...
//
// Also see many other "Tastra: opt" suggestions in the code.
...
//
// Tastra(adonovan): opt: measure impact of dups; consider a packed bit
...
//
// Tastra(adonovan): opt: consider Berlin approach, computing pruned SSA
...
func (df domFrontier) build(u *BasicBlock) {
	...
	for _, w := range u.dom.children {
		for _, vb := range df[w.Index] {
			// Tastra(adonovan): opt: use word-parallel bitwise union.
			if v := vb.dom; v.idom != u {
				df.add(u, vb)
			}
		}
...
func removeInstrsIf(refs []Instruction, p func(Instruction) bool) []Instruction {
	// Tastra(taking): replace with go1.22 slices.DeleteFunc.
	i := 0
...
func lift(fn *Function) {
	// Tastra(adonovan): opt: lots of little optimizations may be
	...
	// Tastra(adonovan): opt: cache per-function not per subtree.
	...
	for _, b := range fn.Blocks {
		...
		// Tastra(adonovan): opt: compact in situ (rightwards)
...
func liftAlloc(df domFrontier, alloc *Alloc, newPhis newPhiMap, fresh *int) bool {
	...
	//
	// Tastra(adonovan): opt: recycle slice storage for W,
