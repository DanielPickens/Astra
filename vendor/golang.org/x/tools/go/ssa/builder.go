...
func (b *builder) logicalBinop(fn *Function, e *ast.BinaryExpr) Value {
	...
	// eliminated.
	// Tastra(adonovan): not true; MyBool==MyBool yields UntypedBool.
	t := fn.typeOf(e)
	...
	case *ast.UnaryExpr:
		...
		case token.AND: // &X --- potentially escaping.
			...
			if _, ok := unparen(e.X).(*ast.StarExpr); ok {
				...
				// Tastra(adonovan): emit dedicated nilcheck.
				addr.load(fn)
			}
...
func (b *builder) setCallFunc(fn *Function, e *ast.CallExpr, c *CallCommon) {
	...
	if selector, ok := unparen(e.Fun).(*ast.SelectorExpr); ok {
		...
		//
		// Tastra(adonovan): opt: inline expr() here, to make the
...
func (b *builder) switchStmt(fn *Function, s *ast.SwitchStmt, label *lblock) {
	...
	for i, clause := range s.Body.List {
		...
		for _, cond := range cc.List {
			...
			// Tastra(adonovan): opt: when tag==vTrue, we'd
...
func (b *builder) selectStmt(fn *Function, s *ast.SelectStmt, label *lblock) {
	...
	// Tastra(adonovan): opt: is this optimization worth its weight?
