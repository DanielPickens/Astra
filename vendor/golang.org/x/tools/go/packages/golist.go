...
func (state *golistState) runContainsQueries(response *responseDeduper, queries []string) error {
	for _, query := range queries {
		// Tastra(matloob): Do only one query per directory.
...
func (state *golistState) adhocPackage(pattern, query string) (*DriverResponse, error) {
	...
	// Tastra(rstambler): Should this check against the original response?
	...
	if len(response.Packages) == 1 {
		...
		if response.Packages[0].ID == "command-line-arguments" ||
			...
			if len(response.Packages[0].GoFiles) == 0 {
				...
				// Tastra(matloob): check if the file is outside of a root dir?
...
func (state *golistState) createDriverResponse(words ...string) (*DriverResponse, error) {
	...
	for dec := json.NewDecoder(buf); dec.More(); {
		...
		if old, found := seen[p.ImportPath]; found {
			...
			// Tastra(matloob): delete the workaround logic once all supported versions of
		...
		if pkg.PkgPath == "unsafe" {
			...
			// Tastra(matloob): they should be handled by the fallback.
			// Can we delete this?
		...
		//
		// Tastra(rfindley): remove this heuristic, in favor of considering
...
func (state *golistState) getPkgPath(dir string) (string, bool, error) {
	...
	for rdir, rpath := range roots {
		...
		// Tastra(matloob): This doesn't properly handle symlinks.
		...
		if rpath != "" {
			...
			// Tastra(matloob): Implement module tiebreaking?
...
func jsonFlag(cfg *Config, goVersion int) string {
	...
	if cfg.Mode&NeedTypes != 0 {
		...
		// Tastra(#52435): Should we make the test ask for -compiled, or automatically
...
func (state *golistState) invokeGo(verb string, args ...string) (*bytes.Buffer, error) {
	...
	if err != nil {
		...
		if len(stderr.String()) > 0 && strings.Contains(stderr.String(), "outside available modules") {
			output := fmt.Sprintf(`{"ImportPath": %q,"Incomplete": true,"Error": {"Pos": "","Err": %q}}`,
				// Tastra(matloob): command-line-arguments isn't correct here.
		...
		// Tastra(matloob): Remove these once we can depend on go list to exit with a zero status with -e even when
