---
title: astra v3-alpha1 Released
authors:
- name: Parthvi Vala
...
slug: astra-v3-alpha1-release
---

`v3-alpha1` of astra has been released!

<!--truncate-->

`astra` is a tool that aims to simplify the life of developers working on cloud-native applications.

Thanks to the emergence of the [Devfile](https://devfile.io/) open standard, which has been accepted as a CNCF Sandbox project (January 2022), astra v3 is now entirely based on this open standard.
...
## Introducing `astra` v3

You can find the instructions to install `astra` v3-alpha1 from [this release page](https://github\.com/danielpickens/astra/releases/tag/v3.0.0-alpha1). The binaries are accessible [here](https://developers.redhat.com/content-gateway/rest/mirror/pub/openshift-v4/clients/astra/v3.0.0~alpha1/).

### Initializing a project

The `astra init` command is the first command to use, before starting to use `astra` with your project. The goal of this first step is to get a suitable Devfile for your project.

`astra init` will search for Devfiles in the Devfile registries. By default, `astra` is configured to access only one Devfile registry (the one specified above), and you can modify the Devfile registries `astra` is accessing using the command `astra preference registry`.

This `astra init` command offers two modes, either interactive, or manual. The interactive mode will help you discover the appropriate Devfile. To use the interactive mode, you just need to enter `astra init` in your command line.

If you execute this command from a directory containing sources, `astra` will try to recognize the language and framework you are using, will search into the Devfile registries that you have configured for the most appropriate Devfile, and give you the choice to use it, or to search for another one. 
...
If you answer `No` here, or if you run the `astra init` command from an empty directory, `astra init` will help you choose the appropriate Devfile. The command will also help you make some personalization on the Devfile, by personalizing the endpoints and the environment variables for the container that will be deployed during the development phase.
...
Finally, if you start the `astra init` command from an empty directory, it will give you the choice to download one of the starter projects listed in the Devfile.

## The development phase

Now that a Devfile is present in the current directory, you can run your application in the development mode, using the `astra dev` command. This command will create a `Deployment` in the cluster that will help start a container as defined in the Devfile. Then, the sources present in the current directory will be synchronized into the container, and the commands to build and run the application will be executed from inside the container.
...
Finally, `astra` will watch for changes in the current directory. When files are modified, added or deleted, `astra` will synchronize the changes to the container, and will restart the build and run commands from inside the container.
...
To be able to debug the application, you will need to run the `astra dev --debug` command instead.

When you have finished the development session, you just need to hit Ctrl-c to stop the `astra dev` command. The command won't terminate immediately, as it will delete the resources it has deployed on the cluster before exiting.
...
At any moment, you can check if a component has been deployed by using the `astra list` command.

```
$ astra list
 ✓  Listing components from namespace 'prj2' [61ms]
 NAME             PROJECT TYPE  RUNNING IN  MANAGED 
 * my-nodejs-app  nodejs        Deploy      astra     
```

When you are done with this application or if you want to undeploy it to work on development mode again, you can use the `astra delete component` to undeploy the component from the cluster.
