...
There are a few ways of binding your application to a database service with the help of astra. The recommended way is with the help of Service Binding Operator(SBO), but it is also possible to bind without it, and this blog will show you how.
...
## Prerequisites:
This blog assumes:
- [astra v3.0.0-beta1](https://github\.com/danielpickens/astra/releases/tag/v3.0.0-beta1)
...
0. We will create a new namespace to deploy our application in, with the help of astra.
```sh
...
```
Make sure `MY_MONGODB_ROOT_USERNAME`, and `MY_MONGODB_ROOT_PASSWORD`  are declared/exported in any new terminal session from where you might run an astra command for this application.
...
5. Run `astra init` to fetch the necessary devfile.
```sh
...
<details>
<summary>If you run <code>astra dev</code> to deploy the application at this point, you will notice that the 'run' command has failed with some logs, this is expected, because like we mentioned before, our Go application is dependent on the MongoDB service and will not function unless it is connected to it.</summary>
...
$ astra dev
 ...
 /  \__/    astra version: v3.0.0-alpha3
 \__/
...
Watching for changes in the current directory /home/pvala/restapi-mongodb-astra
...
The values for _username_, _password_, and _host_ will be passed to devfile.yaml with the `--var` flag when we run the `astra dev` command.
...
7. Run `astra dev` to deploy the application on the cluster.
```sh
astra dev \
...
```sh
$ astra dev --var PASSWORD=$MY_MONGODB_ROOT_PASSWORD --var USERNAME=$MY_MONGODB_ROOT_USERNAME --var HOST="mongodb"
  __
 /  \__     Developing using the restapi Devfile
 \__/  \    Namespace: restapi-mongodb
 /  \__/    astra version: v3.0.0-alpha3
...
* [Binding an external service with astra v3](./2022-06-14-binding-external-service.md)
* [astra add binding](/docs/command-reference/add-binding)