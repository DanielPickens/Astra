---
title: astra with .NET and PostgreSQL on OpenShift
author: Tomas Kral
...
slug: astra-dotnet-postgresql-openshift
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

In this tutorial, I will show you how you can leverage `astra` to develop a .NET application communicating with the PostgreSQL database on OpenShift.
<!--truncate-->


## Prerequisites

- [astra](https://astra.dev/docs/overview/installation) [3.6.0](https://github\.com/danielpickens/astra/releases/tag/v3.6.0) or later
...
- Log in to the cluster using `astra`
  - Log in to the OpenShift Web Console, click on your username in the top right corner and select "Copy login command" ![Copy login command](/img/blog/astra-dotnet-postgresql-openshift/odc_copy_login_command.png)
  - You might get prompted to log in again. After that, you should be able to display the token.
  - Copy `oc login --token=.....` command that you see to your terminal, replace `oc` with `astra`, and execute the command. (If you have `oc` installed, you can leave the command as it is. `astra` and `oc` store the login information at the same place)


## Getting the application source code

We will use [.NET Core Sample App for OpenShift](https://github.com/daniel-pickens/s2i-dotnetcore-persistent-ex)

1. Clone the repository

  ```shell
  git clone https://github.com/daniel-pickens/s2i-dotnetcore-persistent-ex
  ```
...
## Use astra to develop a .NET application on the OpenShift cluster

In this part, we will start our .NET application inside the container on the OpenShift cluster.
Because the app will be built and run inside the container, we don't need to install any .NET tools on our local system. All we need is the `astra` CLI and an OpenShift cluster.
...
<details>
  ...
  ```
    ...
    You can automate this command by executing:
      astra init --name myapp --devfile dotnet60 --devfile-registry DefaultDevfileRegistry
...
`astra init` command will guide you through selecting the correct Devfile stack and some basic configuration. For purposes of this tutorial, you can just accept the default values.


:::caution
Please pay attention to the version of the `dotnet` Devfile that `astra` suggests.
`astra` might not be able to detect the .NET version that our application is using correctly.
If that happens, you need to tell astra that it is not correct and select the correct version manually.
:::

#### Edit devfile

We need to modify `devfile.yaml` that got downloaded by `astra init` command to ensure that our application starts correctly.
...
We have our application source code and the `devfile.yaml` that describes the environment in which the application should be executed. We can now start the application on the cluster. Just run `astra dev` in the project directory.
...
`astra dev` performs the following actions:
...
1. When the application is successfully running in the container, `astra` sets the port forwarding between your local machine and the container to ensure we can easily access our application on the local port. In this case, it is port `20001`.

Now you can access the application on [http://localhost:20001](http://localhost:20001).

![App running with In Memory DB](/img/blog/astra-dotnet-postgresql-openshift/app_running_inmemory_db.png)
...
You can leave `astra dev` running in the terminal and open a new terminal window to continue with the next steps. Make sure that you run all `astra` commands in the root folder of our .NET application (where you have `devfile.yaml` and `RazorPagesContacts` files).
...
<details>
  ...
  <ol>
    ...
    <li>
      ...
      <img src="/img/blog/astra-dotnet-postgresql-openshift/odc_db_step_1.png" alt="ODC step 1" />
    </li>
    <li>
      Click on the <b>+Add</b> button in the main menu and select <b>Database</b> option from the <b>Developer Catalog</b> section.
      <img src="/img/blog/astra-dotnet-postgresql-openshift/odc_db_step_2.png" alt="ODC step 2" />
    </li>
    <li>
      In the <b>Database</b> section select <b>PostgreSQL</b> option. Make sure to select the one using <b>Templates</b>.
      <img src="/img/blog/astra-dotnet-postgresql-openshift/odc_db_step_3.png" alt="ODC step 3" />
    </li>
    <li>
      Click on the <b>Instantiate Template</b> button.
      <img src="/img/blog/astra-dotnet-postgresql-openshift/odc_db_step_4.png" alt="ODC step 4" />
    </li>
    <li>
      For our use we can use the default values. Just click on the <b>Create</b> button.
      <img src="/img/blog/astra-dotnet-postgresql-openshift/odc_db_step_5.png" alt="ODC step 5" />
    </li>
  </ol>
  </div>
</details>
...
We will use one of the newest `astra` and Devfile features - `container-overrides`.
...
As you modify the `devfile.yaml`, you can check the terminal window where you have `astra dev` running. You should see that `astra` is automatically handling the changes and reloading the application.
...
![App running with PostgreSQL DB](/img/blog/astra-dotnet-postgresql-openshift/app_running_postgresql_db.png)
