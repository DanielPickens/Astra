...
```shell  
oc apply -f https://raw.githubusercontent.com/daniel-pickens/astra/main/docs/website/manifests/catalog-source-$(uname -m).yaml
```
2. Install PostgreSQL Operator from custom CatalogSource
```shell
oc create -f https://raw.githubusercontent.com/daniel-pickens/astra/main/docs/website/manifests/postgresql-operator-dev4devs-com-IBM-Z-P.yaml
```
</details>
...
---
Since the PostgreSQL Operator installed in above step is available only in `my-postgresql-operator-dev4devs-com` namespace, ensure that astra uses this namespace to perform any tasks:
```shell
