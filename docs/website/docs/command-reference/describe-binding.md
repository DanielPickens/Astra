...
`astra describe binding` may therefore not work as expected.

:::

`astra describe binding` command is useful for getting information about service bindings.

This command supports the service bindings added with the command `astra add binding` and bindings added manually to the Devfile, using a `ServiceBinding` resource from one of these apiVersion:
...
Start "astra dev" first to see binding information.
```
</details>
...
```console
astra describe binding --name <component_name>
```
...
```shell
$ astra describe binding --name my-nodejs-app-redis-standalone
Service Binding Name: my-nodejs-app-redis-standalone
Services:
 •  redis-standalone (Redis.redis.redis.opstreelabs.in)
Bind as files: false
