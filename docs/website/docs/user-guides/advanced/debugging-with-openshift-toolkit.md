...
## OpenShift Toolkit
OpenShift Toolkit is an IDE plugin available on [VS Code](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-openshift-connector) and [JetBrains](https://plugins.jetbrains.com/plugin/12030-openshift-toolkit-by-red-hat/) IDEs, that allows you to do all things that `astra` does, i.e. create, test, debug and deploy cloud-native applications on a cloud-native environment in simple steps.
`astra` enables this plugin to do what it does.
...
2. You have [initialized an application with `astra`](/docs/command-reference/init), for example [the Node.JS quickstart application](../quickstart/nodejs.md#step-2-initializing-your-application-astra-init).
:::note
...
The debug endpoint name must be named **debug** or be prefixed by **debug-** so that `astra` can recognize it as a debug port.
