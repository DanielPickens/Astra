...
We need to provide a configuration file `astra-config` in a configuration directory `.astra`, that will hold specific configuration that `astra` will use on execution. This configuration might modify how the CLI work or how the component will work on the platform.

### Scopes of configuration

`astra` CLI configuration needs to be preserved in configuration files. As a developer might work on multiple
...
Every component can be configured independently. We call this local configuration for the component as it only affects the component it references. This configuration will be stored in a file in `.astra` specific directory located at the root directory of the component's source code. In this way, the configuration file can be added to version control system and preserved along with the application. If any other developer checks out the same source code he will get the same configuration as the original developer.

We propose to have a `.astra` folder in the root of the application's source code directory to store every astra local configurations.

```bash
<app-dir>/.astra/astra-config
```

A good representation of this is a developer that while on the development process realizes his application requires 2 GB of memory to run appropriately, he might decide to save that information in this configuration file. At a later moment, any other developer (or even him in the future) that decides to work on the same application will have this configuration definition along with his source code. `astra` should take into account this configuration definition and materialize when appropriate. The type of configuration, format and applicability will be described later in this document.

#### Global configuration

When working with multiple components it might be tedious to configure each and every component, or a user wants to have a default configuration value applied to each and every component he works with when no explicit (local) configuration exists. For this purpose, there should be a file, stored in the user's home directory's astra specific folder, that should contain this configuration definitions. This configuration is global to every component the user works with.

We propose to have a `.astra` folder in the root of the users home directory to store every astra global configurations.

```bash
$HOME/.astra/astra-config
```

#### User explicit configuration

Often times a user wants to use different configuration values than the provided in the configuration file. We need to provide a way to allow a developer to specify this new values he want to use. For that, the user needs to explicitly provide the values as CLI arguments to the specific `astra` command.

An example of this would be:

```bash
astra create wildfly --memory 2GB
```

In this previous example, the user is explicitly instructing `astra` to use 2GB of memory when creating a component based on wildfly.

User explicit configuration will be stored in a local configuration file also in the astra`s specific local folder:

```bash
<app-dir>/.astra/astra-config.local
```

This user explicit configuration will be used in any subsequent command, avoiding the user the need to provide the flag every time. This file serves as configuration overrides. When explicitly setting additional configuration for a component, this configuration will be saved in the `.astra/astra-config.local`. Every time a flag is provided, it will overwrite the value in that file if it already existed, as new override value.

There is no specific command that will manage this file. If a user needs to change/delete any value, he will need to manually edit this file. A value will be set in this file when astra is used.

There's an additional purpose to this file an is for `astra` to automatically recognize the applied configuration to a component when the user changes directory into the application's source code dir, the same way as git does with .git directory and config.

**NOTE:** The automatic detection of `.astra` configuration when a user changes directories, and the corresponding `astra` behavior, will be described in a different proposal.
...
### `astra` CLI behavior configuration

Like many tools, there is configuration related to how the tool behaves itself. An example of this would be a timeout that the CLI should apply when issuing commands to the server, or the ability to enable/disable checks for new versions. This configuration will always be global, and stored relative to the user's astra directory in the user's home directory. By default this configuration file will live in `$HOME/.astra/astra-config`

The [format](#astra-file-format) of this file is described later in the proposal.

### Application related configuration

Sometimes what we want to configure is how the application/component will be deployed. Some components will
require special characteristics, like a specific amount of memory. Some other times, what we want is to instruct `astra` to use an specific value to name our component if none is explicitly provided. This configuration will be stored locally to the component, along with the source code, in an `astra-config` configuration file, in an `.astra` specific directory, that can be saved into the version control system used, and share by anyone using the same application code.
...
One of the goals of `astra` is to be easy to use for developers. This forces to translate some configuration possible in the underlying technology (OpenShift/Kubernetes) into a friendlier name that developers could easily understand. An example of this would be the amount of memory a component needs. In OpenShift, this is reflected by `limits.memory` and `requests.memory`, and depending on how you set these values, influences the QoS of the deployment. In `astra` these should be simplified to `memory`, `min-memory` and `max-memory`. These are terms that any developer will most likely understand without looking at the documentation.
...
We need to define and delimit what information will be stored in this file. Configuration which can be local and global will be denoted. Otherwise, the configuration will only be local to the application's astra-config file.
...
* astraignore: (local/global) list of files/patterns to ignore by default if no .astraignore file is provided, or when it's cerated. **This configuration can be global, as the developer might want to always ignore specific files (e.g. .git, .astra, ...) when pushing.**

#### <a name="astra-file-format"></a> `astra-config` file format
...
Nevertheless, `astra` will provide a way to manipulate this file via a CLI command, as described in [managing configuration](#managing-configuration)

### <a name="managing-configuration"></a> Managing configuration

Developers will need to add configuration values to the `.astra/astra-config` config files. This configuration management will be managed by a new command in `astra` CLI. As configuration will be of different types, there will be needs to additional verbs (to the regular ones) for managing this configuration.

#### Creating configuration

There might be times that a developer would want to add/edit/remove values from the `.astra/astra-config` configuration file. There should be a command to manage this file in the `astra` CLI. That command should be the same used to touch configuration of the global `astra` config file located in the users's home directory.

Currently that command is:

```bash
astra utils config [create|update|delete|list|add|remove|get|describe] [KEY] [VALUES] [--local|--local]
```

In order to create configuration, we should call astra with the appropriate key and value or list of values to apply, as well as the scope for this configuration to be saved (local or global).

```bash
astra utils config set memory 2Gi               # Set's component memory to 2 GB local config
astra utils config add astraignore .astra --local   # Adds a value to astraignore list local config
astra utils config add astraignore .astra --global  # Adds a value to astraignore list global config
```
...
**NOTE**: When creating a component, no configuration file will be created. If a developer thinks a configuration value is worth being stored in the `.astra/astra-config` configuration file, he will need to use the previous command to create these files (whether global or local doesn't matter).
...
```bash
astra utils config list --global      # List all global config
astra utils config get memory         # List the value for memory config key
```
...
```bash
astra utils config update app-name example   # Updates app-name value
astra utils config add astraignore .eclipse    # Adds .eclipse to the list of ignored files
```
...
```bash
astra utils config delete memory --local                # Removes configuration for memory
astra utils config remove astraignore .git --local        # Removes .git from the list of values for astraignore
```
...
```bash
astra utils config describe memory        #Provides a description of the memory configuration key
```
