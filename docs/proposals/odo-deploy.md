# Support build and deployment of application container image using astra
...
It would be useful to start the design/development of a simpler version of `astra deploy` with devfile 2.0.0 that covers:
...
## Motivation
`astra` is limited to development inner-loop for a project and there is no support for outer-loop - build a slim/production container image for your application and deploy the build container. It would be very useful for developers to be able to try inner-loop and then transition over to the outer-loop using astra. The outer-loop information could be provided by the application stack (devfile) to avoid developers having to worry about these aspects. 

`astra deploy` can be a good way to assure the developer that their application can be built and deployed successfully using the build/deploy guidance that comes from devfile.

It is not meant to replace GitOps/pipeline-based deployments for governed environments (test, stage, production). However, both `astra deploy` and `astra pipelines` must honour the build and deployment information provided by the application stack (devfile).
...
User flow: 
1. `astra create <component> <mycomponent>` - This initializes astra component in the current directory.
1. Edit the project source code to develop the application.
1. `astra url create` - This stores URL information (host, port etc.) for accessing the application on the cluster (if not done already).
1. `astra push` - This runs the application source code using inner-loop instructions from devfile.
1. Validate the running application is working as intended.
1. Iterate over steps 2 and beyond (as needed). 
1. `astra deploy` - This would build a new clean image and deploy it to the target cluster using outer-loop instructions from devfile and user-provided arguments.
1. Validate the deployed application is working as intended.
1. Iterate over steps 2 and beyond (as needed).
1. Optionally, run `astra deploy delete` to clean up resources created with `astra deploy`.
1. Push your code to Git - ready for sharing with your team and for CI/CD pipelines to take over.

## User stories

### Initial build and deploy support to astra - https://github\.com/danielpickens/astra/issues/3300

## Design overview
`astra deploy` could provide developers with a way to build a container image for their application and deploy it on a target Kubernetes deployment using the build/deploy guidance provided by the devfile.
...
`astra deploy delete` could provide developers with a way to clean up any existing deployment of the application. 
...
#### astra deploy delete
This command will delete any resources created by astra deploy.

### Detailed design: 

### Devfile

For the initial implementation, we could use devfile v2.0.0 and capture basic outer-loop information as `metadata` on the devfile. `astra` could look for  specific keys, while other tools like Che could ignore them.
...
- Replace templated text in the deployment manifest with relevant values:
    - {{.COMPONENT_NAME}}: name of astra component/microservice
...
- Save the deployment manifest in `.astra/env/` folder.
- Provide the user with a URL for accessing the deployed application.

### astra deploy delete
This command will perform the following actions:
- Check if there is an existing deployment for the app (can use the saved deployment manifest in `.astra/env/` folder)
...
- If a devfile does not provide deployment manifest, astra can perhaps create a manifest in the way it does for inner-loop. This will mean devfile creators do not need to provide a deployment manifest if they do not care so much about the deployment aspect.

- Once `astra link` and service binding is supported by astra and devfiles v2, we could use the same service binding information for `astra deploy`.