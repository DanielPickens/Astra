...
const (
	GlobalConfigEnvName  = "GLOBALastraCONFIG"
	configFileName       = "preference.yaml"
	preferenceKind       = "Preference"
	preferenceAPIVersion = "astra.dev/v1alpha1"
	...
	OldDefaultDevfileRegistryURL = "https://github.com/astra-devfiles/registry"

	// DefaultRegistryCacheTime is time (in minutes) for how long astra will cache information from Devfile registry
...
var RegistryCacheTimeSettingDescription = fmt.Sprintf("For how long (in Duration) astra will cache information from the Devfile registry (Default: %s)", DefaultRegistryCacheTime)

// EphemeralSettingDescription adds a description for EphemeralSourceVolume
var EphemeralSettingDescription = fmt.Sprintf("If true, astra will create an emptyDir volume to store source code (Default: %t)", DefaultEphemeralSetting)

// ConsentTelemetrySettingDescription adds a description for TelemetryConsentSetting
var ConsentTelemetrySettingDescription = fmt.Sprintf("If true, astra will collect telemetry for the user's astra usage (Default: %t)\n\t\t    For more information: https://developers.redhat.com/article/tool-data-collection", DefaultConsentTelemetrySetting)
