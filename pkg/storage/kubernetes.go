...
func (k kubernetesClient) Create(storage Storage) error {
	...
	labels := astralabels.GetLabels(k.componentName, k.appName, k.runtime, astralabels.ComponentDevMode, false)
	astralabels.AddStorageInfo(labels, storage.Name, strings.Contains(storage.Name, astraSourceVolume))
...
func (k kubernetesClient) List() (StorageList, error) {
	if k.deployment == nil {
		var err error
		selector := astralabels.GetSelector(k.componentName, k.appName, astralabels.ComponentAnyMode, true)
	...
	for _, container := range k.deployment.Spec.Template.Spec.Containers {
		for _, volumeMount := range container.VolumeMounts {
			...
			if (!ok && initOK) ||
				volumeMount.Name == astraSourceVolume ||
	...
	selector := astralabels.SelectorBuilder().WithComponent(k.componentName).WithoutSourcePVC(astraSourceVolume).Selector()
	...
	for _, pvc := range pvcs {
		...
		for _, volumeMount := range volumeMounts {
			if volumeMount.Name == pvc.Name+"-vol" {
				...
				storage = append(storage, NewStorageWithContainer(astralabels.GetDevfileStorageName(pvc.Labels), size.String(), volumeMount.Spec.Path, volumeMount.Spec.ContainerName, nil))
			}
		}
		if !found {
