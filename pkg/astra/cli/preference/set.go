...
var (
	setLongDesc = ktemplates.LongDesc(`Set an individual value in the astra preference file.  
%[1]s`)
...
func (o *SetOptions) Run(ctx context.Context) (err error) {
	...
	if !o.forceFlag {
		if isSet := o.clientset.PreferenceClient.IsSet(o.paramName); isSet {
			// Tastra: could add a logic to check if the new value set by the user is not same as the current value
			var proceed bool
...
// NewCmdSet implements the config set astra command
func NewCmdSet(ctx context.Context, name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	...
	preferenceSetCmd := &cobra.Command{
		Use:   name,
		Short: "Set a value in the astra preference file",
		...
		Example: func(exampleString, fullName string) string {
			...
			if err != nil {
				util.LogErrorAndExit(err, "unable to set preference, something is wrong with astra, kindly raise an issue at https://github\.com/danielpickens/astra/issues/new?template=Bug.md")
			}
