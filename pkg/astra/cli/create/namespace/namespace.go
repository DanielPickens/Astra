...
// NamespaceCreateOptions encapsulates the options for the astra namespace create command
...
func (nco *NamespaceCreateOptions) Run(ctx context.Context) (err error) {
	...
	L:
		for {
			select {
			case <-timeOut:
				return fmt.Errorf("timeout while waiting for %s %q to be ready; you can change the timeout preference by running `astra preference set timeout <duration>`", nco.commandName, nco.namespaceName)
			default:
