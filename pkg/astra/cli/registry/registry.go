...
// ListOptions encapsulates the options for the astra registry command
...
func (o *ListOptions) Validate(ctx context.Context) error {
	if o.devfileList.DevfileRegistries == nil {
		...
		return fmt.Errorf("no registry in preferences, please add a registry using 'astra preference add registry' command")
	}
...
func NewCmdRegistry(name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	...
	astrautil.SetCommandGroup(listCmd, astrautil.MainGroup)
...
`,
				...
				log.Sbold("Supported astra Features"),
