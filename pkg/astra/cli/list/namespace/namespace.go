...
// NamespaceListOptions encapsulates the options for the astra list project command
...
// Run contains the logic for the astra list project command
...
// NewCmdNamespaceList implements the astra list project command.
...
func HumanReadableOutput(w io.Writer, o api.ResourcesList, commandName string) error {
	if len(o.Namespaces) == 0 {
		return fmt.Errorf("you are not a member of any %[1]ss. You can request a %[1]s to be created using the `astra create %[1]s <%[1]s_name>` command", commandName)
	}
