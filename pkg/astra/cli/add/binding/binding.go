...
func (o *AddBindingOptions) Run(ctx context.Context) error {
	...
	withDevfile := astrautil.CheckPathExists(o.clientset.FS, location.DevfileLocation(o.clientset.FS, astracontext.GetWorkingDirectory(ctx)))
	...
	if withDevfile {
		...
		exitMessage := "Run `astra dev` to create it on the cluster."
		if len(o.flags) == 0 {
			kindGroup := strings.ReplaceAll(strings.ReplaceAll(splitService[1], "(", ""), ")", "")
			exitMessage += fmt.Sprintf("\nYou can automate this command by executing:\n  astra add binding --service %s.%s --name %s", serviceName, kindGroup, bindingName)
			if ns != "" {
