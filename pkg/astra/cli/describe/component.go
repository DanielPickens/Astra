...
func (o *ComponentOptions) Complete(ctx context.Context, cmdline cmdline.Cmdline, args []string) (err error) {
	...
	// 1. Name is not passed, and astra has access to devfile.yaml; Name is not passed so we assume that astra has access to the devfile.yaml
	if o.nameFlag == "" {
	...
	// 2. Name is passed, and astra does not have access to devfile.yaml; if Name is passed, then we assume that astra does not have access to the devfile.yaml
...
func (o *ComponentOptions) RunForJsonOutput(ctx context.Context) (out interface{}, err error) {
	result, _, err := o.run(ctx) // Tastra(feloy) handle warning
	if clierrors.AsWarning(err) {
		err = nil
	}
	return result, err
...
func printHumanReadableOutput(ctx context.Context, cmp api.Component, devfileObj *parser.DevfileObj) error {
	...
	log.Info("Supported astra features:")
	if cmp.DevfileData != nil && cmp.DevfileData.SupportedastraFeatures != nil {
		log.Printf("Dev: %v", cmp.DevfileData.SupportedastraFeatures.Dev)
		log.Printf("Deploy: %v", cmp.DevfileData.SupportedastraFeatures.Deploy)
		log.Printf("Debug: %v", cmp.DevfileData.SupportedastraFeatures.Debug)
	} else {
...
// NewCmdComponent implements the component astra sub-command
