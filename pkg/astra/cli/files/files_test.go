...
func TestGetFilesGeneratedByastra(t *testing.T) {
	type args struct {
	...
	for _, tt := range []test{
		{
			name: "error reading .astra/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
			...
			wantErr: true,
			want:    []string{util.DotastraDirectory},
		},
		{
			name: "non-existing .astra/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: func(name string, flag int, perm os.FileMode) (filesystem.File, error) {
		...
		{
			name: "empty .astra/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
			...
			setup: func() error {
				_, err := fakeFs.Create(filepath.Join("/path/to/root/directory/2", util.DotastraDirectory, _dotastraGenerated))
				return err
			},
			wantErr: false,
			want:    []string{util.DotastraDirectory},
		...
		{
			name: ".astra/generated file with 1 line",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: fakeFs.OpenFile,
			...
			setup: func() error {
				return fakeFs.WriteFile(
					filepath.Join("/path/to/root/directory/3", util.DotastraDirectory, _dotastraGenerated),
					[]byte("devfile.yaml"),
					0644,
				)
			},
			wantErr: false,
			want:    []string{util.DotastraDirectory, "devfile.yaml"},
		},
		{
			name: ".astra/generated file with multiple lines",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: fakeFs.OpenFile,
			...
			setup: func() error {
				return fakeFs.WriteFile(
					filepath.Join("/path/to/root/directory/4", util.DotastraDirectory, _dotastraGenerated),
					[]byte("devfile.yaml\n.gitignore\n"),
					0644,
				)
			},
			wantErr: false,
			want:    []string{util.DotastraDirectory, "devfile.yaml", ".gitignore"},
		},
		{
			name: ".astra/generated file with blank lines that should be ignored",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
						openFile: fakeFs.OpenFile,
			...
			setup: func() error {
				return fakeFs.WriteFile(
					filepath.Join("/path/to/root/directory/5", util.DotastraDirectory, _dotastraGenerated),
					[]byte("\n\ndevfile.yaml\n\n\t\n.gitignore\n\n/path/to/a/file\na-path with spaces\n"),
					0644,
				)
			},
			wantErr: false,
			want:    []string{util.DotastraDirectory, "devfile.yaml", ".gitignore", "/path/to/a/file", "a-path with spaces"},
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := GetFilesGeneratedByastra(tt.args.fsProvider(), tt.args.rootDir)
			...
			if diff := cmp.Diff(tt.want, got); diff != "" {
				t.Errorf("GetFilesGeneratedByastra() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

func TestReportLocalFileGeneratedByastra(t *testing.T) {
	type args struct {
		fsProvider func() filesystem.Filesystem
		rootDir    string
		filename   string
	...
	for _, tt := range []test{
		{
			name: "error when creating .astra directory",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
		...
		{
			name: "error when opening .astra/generated file",
			args: args{
				fsProvider: func() filesystem.Filesystem {
					return partialFs{
		...
		t.Run(tt.name, func(t *testing.T) {
			err := ReportLocalFileGeneratedByastra(tt.args.fsProvider(), tt.args.rootDir, tt.args.filename)
			...
			got, err := fakeFs.ReadFile(filepath.Join(tt.args.rootDir, util.DotastraDirectory, _dotastraGenerated))
			if err != nil {
