...
import (
	...
	"github\.com/danielpickens/astra/pkg/auth"
...
// LoginOptions encapsulates the options for the astra command
...
func (o *LoginOptions) Complete(ctx context.Context, cmdline cmdline.Cmdline, args []string) (err error) {
	if len(args) == 1 {
		...
		// astra login -u developer -p developer https://api.crc.testing:6443
		// astra login --token=<some-token> https://api.crc.testing:6443
		o.server = args[0]
	}
	return
}
...
func (o *LoginOptions) Validate(ctx context.Context) (err error) {
	if o.server != "" && o.serverFlag != "" && o.server != o.serverFlag {
		...
		// * astra errors *if* the values are different
		// * astra silently continues if the values are same
...
// NewCmdLogin implements the astra command
func NewCmdLogin(name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	...
	loginCmd.SetUsageTemplate(astrautil.CmdUsageTemplate)
