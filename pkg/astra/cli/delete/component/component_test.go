...
func TestComponentOptions_deleteNamedComponent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx = astracontext.WithComponentName(ctx, "a-name")
...
func TestComponentOptions_deleteDevfileComponent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := astracontext.WithNamespace(context.Background(), projectName)
			ctx = astracontext.WithApplication(ctx, "app")
			ctx = astracontext.WithWorkingDirectory(ctx, workingDir)
			ctx = astracontext.WithComponentName(ctx, compName)
			ctx = astracontext.WithEffectiveDevfileObj(ctx, &info)
...
// populateWorkingDir populates the working directory with .astra and devfile.yaml, and returns envinfo
