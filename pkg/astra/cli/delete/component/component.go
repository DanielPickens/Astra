...
type ComponentOptions struct {
	...
	// withFilesFlag controls whether files generated by astra should be deleted as well.
	withFilesFlag bool
...
func (o *ComponentOptions) printRemainingResources(ctx context.Context, remainingResources []unstructured.Unstructured) {
	...
	log.Finfof(o.clientset.Stdout, "If you want to delete those, execute `astra delete component --name %s --namespace %s`\n", componentName, namespace)
}
...
func (o *ComponentOptions) deleteDevfileComponent(ctx context.Context) ([]unstructured.Unstructured, error) {
	...
	if o.withFilesFlag {
		filesToDelete, err = getFilesCreatedByastra(o.clientset.FS, ctx)
		if err != nil {
			return nil, err
		}
	}
	...
	if hasFilesToDelete {
		o.printFileCreatedByastra(filesToDelete, hasClusterResources)
	}
	...
	if proceed {
		...
		if o.withFilesFlag || len(orphans) > 0 {
			// Delete files
			remainingFiles := o.deleteFilesCreatedByastra(o.clientset.FS, filesToDelete)
...
// getFilesCreatedByastra gets the list of all files that were initially created by astra.
func getFilesCreatedByastra(filesys filesystem.Filesystem, ctx context.Context) ([]string, error) {
	workingDir := astracontext.GetWorkingDirectory(ctx)
	filesToDelete, err := files.GetFilesGeneratedByastra(filesys, workingDir)
	if err != nil {
		return nil, err
	}
...
// getOrphanDevstateFiles gets the list of all Devstate files for which no astra process exists
...
func (o *ComponentOptions) printFileCreatedByastra(files []string, hasClusterResources bool) {
	if len(files) == 0 {
		return
	}
...
// deleteFilesCreatedByastra deletes all the files that were created initially by astra.
// It returns a slice of files that could not be deleted.
func (o *ComponentOptions) deleteFilesCreatedByastra(filesys filesystem.Filesystem, files []string) (notDeleted map[string]error) {
...
func NewCmdComponent(ctx context.Context, name, fullName string, testClientset clientset.Clientset) *cobra.Command {
	...
	componentCmd.Flags().StringVar(&o.runningInFlag, "running-in", "",
		"Delete resources running in the specified mode, optional. By default, all resources created by astra for the component are deleted.")
	componentCmd.Flags().BoolVarP(&o.withFilesFlag, "files", "", false, "Delete all files and directories generated by astra. Use with caution.")
