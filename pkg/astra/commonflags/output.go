...
// We use "flag" in order to make this accessible throughtout ALL of astra, rather than the
...
func CheckMachineReadableOutputCommand(envconfig *config.Configuration, cmd *cobra.Command) error {
	...
	if log.IsJSON() {
		...
		// Override the logging level by the value (if set) by the astra_LOG_LEVEL env
		// The "-v" flag set on command line will take precedence over astra_LOG_LEVEL env
		v := flag.CommandLine.Lookup("v").Value.String()
		if envconfig.astraLogLevel != nil && v == "0" {
			_ = flag.CommandLine.Set("v", strconv.Itoa(*envconfig.astraLogLevel))
		}
	}
	return nil
}
