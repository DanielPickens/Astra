...
import (
	...
	"github\.com/danielpickens/astra/pkg/dev/kubedev/utils"
	...
	"github\.com/danielpickens/astra/pkg/service"
	storagepkg "github\.com/danielpickens/astra/pkg/storage"
...
func (o *DevClient) createComponents(ctx context.Context, parameters common.PushParameters, componentStatus *watch.ComponentStatus) (bool, error) {
	var (
		appName       = astracontext.GetApplication(ctx)
	...
	// Set the mode to Dev since we are using "astra dev" here
	runtime := component.GetComponentRuntimeFromDevfileMetadata(parameters.Devfile.Data.GetMetadata())
	labels := astralabels.GetLabels(componentName, appName, runtime, astralabels.ComponentDevMode, false)
	...
	// and the service binding secrets must be deleted manually since they are created by astra
	...
	_, err = o.pushDevfileKubernetesComponents(ctx, parameters, labels, astralabels.ComponentDevMode, ownerReference)
	if err != nil {
		return false, err
	}
...
func (o *DevClient) getComponentDeployment(ctx context.Context) (*appsv1.Deployment, bool, error) {
	...
	// Since `astra deploy` can theoretically deploy a deployment as well with the same instance name
	// we make sure that we are retrieving the deployment with the Dev mode, NOT Deploy.
	selectorLabels := astralabels.GetSelector(componentName, appName, astralabels.ComponentDevMode, true)
...
func (o *DevClient) createOrUpdateComponent(
	...
	// Set the labels
	labels := astralabels.GetLabels(componentName, appName, runtime, astralabels.ComponentDevMode, true)

	annotations := make(map[string]string)
	astralabels.SetProjectType(annotations, component.GetComponentTypeFromDevfileMetadata(parameters.Devfile.Data.GetMetadata()))
	astralabels.AddCommonAnnotations(annotations)
...
func (o DevClient) getRemoteResourcesNotPresentInDevfile(ctx context.Context, parameters common.PushParameters, selector string) (objectsToRemove, serviceBindingSecretsToRemove []unstructured.Unstructured, err error) {
	...
	for _, remoteK := range allRemoteK8sResources {
		if !astralabels.IsCoreComponent(remoteK.GetLabels()) {
			// ignore the resources that are already set for deletion
			// ignore the resources that do not have projecttype annotation set; they will be the resources that are not created by astra
			...
			if remoteK.GetDeletionTimestamp() != nil && !astralabels.IsProjectTypeSetInAnnotations(remoteK.GetAnnotations()) {
				continue
			}
...
func (o *DevClient) pushDevfileKubernetesComponents(
	...
	// from astra standpoint, these components contain yaml manifest of ServiceBinding
...
func (o *DevClient) buildVolumes(ctx context.Context, parameters common.PushParameters, containers, initContainers []corev1.Container) ([]corev1.Volume, error) {
	...
	err := storage.HandleastraSourceStorage(o.kubernetesClient, storageClient, componentName, o.prefClient.GetEphemeralSourceVolume())
	if err != nil {
		return nil, err
	}
	...
	// These PVCs have been created earlier with `storage.HandleastraSourceStorage` and `storagepkg.Push`
	pvcs, err := o.kubernetesClient.ListPVCs(fmt.Sprintf("%v=%v", "component", componentName))
	if err != nil {
		return nil, err
	}
	...
	// astraSourcePVCName will be empty when Ephemeral preference is true
	astraSourcePVCName, volumeNameToVolInfo, err := storage.GetVolumeInfos(pvcs)
	if err != nil {
		return nil, err
	}

	// Add the volumes for the projects source and the astra-specific directory
	astraMandatoryVolumes := utils.GetastraContainerVolumes(astraSourcePVCName)
	allVolumes = append(allVolumes, astraMandatoryVolumes...)

	// Add the volumeMounts for the project sources volume and the astra-specific volume into the containers
	utils.AddastraProjectVolume(containers)
	utils.AddastraMandatoryVolume(containers)
