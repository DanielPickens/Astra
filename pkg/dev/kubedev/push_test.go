...
func TestCreateOrUpdateComponent(t *testing.T) {
	...
	deployment := v1.Deployment{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels:      astralabels.Builder().WithComponentName(testComponentName).WithAppName(testAppName).Labels(),
			Annotations: astralabels.Builder().WithProjectType("").Labels(),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.componentType != "" {
				astralabels.SetProjectType(deployment.Annotations, string(tt.componentType))
				comp = astraTestingUtil.GetFakeContainerComponent("component")
			}
			...
			ctx = astracontext.WithApplication(ctx, "app")
			ctx = astracontext.WithComponentName(ctx, "my-component")
			ctx = astracontext.WithDevfilePath(ctx, "/path/to/devfile")
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				...
				appName:       "app",
				deployment:    astraTestingUtil.CreateFakeDeployment("nodejs", false),
			},
			args: args{
				labels:      astraTestingUtil.CreateFakeDeployment("nodejs", false).Labels,
				annotations: nil,
			},
			want:    generator.GetObjectMeta("nodejs", "project-0", astraTestingUtil.CreateFakeDeployment("nodejs", false).Labels, nil),
			wantErr: false,
		},
		{
			...
			want:    generator.GetObjectMeta(namespacedKubernetesName, "project-0", astraTestingUtil.CreateFakeDeployment("nodejs", false).Labels, nil),
			wantErr: false,
		},
		{
			...
			args: args{
				labels:      astraTestingUtil.CreateFakeDeployment("nodejs", false).Labels,
				annotations: astralabels.Builder().WithMode(astralabels.ComponentDevMode).Labels(),
			},
			want:    generator.GetObjectMeta("nodejs", "project-0", astraTestingUtil.CreateFakeDeployment("nodejs", false).Labels, astralabels.Builder().WithMode(astralabels.ComponentDevMode).Labels()),
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.Background()
			ctx = astracontext.WithApplication(ctx, "app")
			ctx = astracontext.WithComponentName(ctx, "nodejs")
			ctx = astracontext.WithDevfilePath(ctx, "/path/to/devfile")
