...
func TestAddastraProjectVolume(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			AddastraProjectVolume(tt.containers)
			...
			for wantContainerName, wantMountPath := range tt.volMount {
				...
				for _, container := range tt.containers {
					if container.Name == wantContainerName {
						for _, volMount := range container.VolumeMounts {
							if volMount.Name == storage.astraSourceVolume && volMount.MountPath == wantMountPath {
								matched = true
							}
						}
					}
				...
				if !matched {
					t.Error("TestAddastraProjectVolume error: did not match the volume mount for astra-projects")
				}
			}
		})
	}
...
func TestAddastraMandatoryVolume(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			AddastraMandatoryVolume(tt.containers)
