...
func TestGetVolumeInfos(t *testing.T) {
	tests := []struct {
		...
		wantastraSourcePVCName string
		wantInfos            map[string]VolumeInfo
		wantErr              bool
	}{
		{
			name: "astra-projects is not found",
			pvcs: []corev1.PersistentVolumeClaim{
				{
		...
		},
		{
			...
			wantastraSourcePVCName: "pvc1",
			wantInfos:            map[string]VolumeInfo{},
		},
		{
			name: "astra-projects is found and other pvcs",
			pvcs: []corev1.PersistentVolumeClaim{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "pvc1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			astraSourcePVCName, infos, err := GetVolumeInfos(tt.pvcs)
			...
			if diff := cmp.Diff(tt.wantastraSourcePVCName, astraSourcePVCName); diff != "" {
				t.Errorf("GetVolumeInfos() wantastraSourcePVCName mismatch (-want +got):\n%s", diff)
			}
