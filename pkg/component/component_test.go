...
func TestListAllClusterComponents(t *testing.T) {
	const astraVersion = "v3.0.0-beta3"
	res1 := getUnstructured("dep1", "deployment", "v1", "Unknown", "", "Unknown", "my-ns")
	res2 := getUnstructured("svc1", "service", "v1", "astra", astraVersion, "nodejs", "my-ns")
	res3 := getUnstructured("dep1", "deployment", "v1", "Unknown", "", "Unknown", "my-ns")
	res3.SetLabels(map[string]string{})

	commonLabels := labels.Builder().WithComponentName("comp1").WithManager("astra").WithManagedByVersion(astraVersion)

	resDev := getUnstructured("depDev", "deployment", "v1", "astra", astraVersion, "nodejs", "my-ns")
	labelsDev := commonLabels.WithMode("Dev").Labels()
	resDev.SetLabels(labelsDev)

	resDeploy := getUnstructured("depDeploy", "deployment", "v1", "astra", astraVersion, "nodejs", "my-ns")
	...
	tests := []struct {
		...
		{
			name: "1 non-astra resource returned with Unknown",
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
		...
		{
			name: "0 non-astra resource without instance label is not returned",
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					var resources []unstructured.Unstructured
		...
		{
			name: "1 non-astra resource returned with Unknown, and 1 astra resource returned with astra",
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					var resources []unstructured.Unstructured
