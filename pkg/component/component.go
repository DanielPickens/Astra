...
func GatherName(contextDir string, devfileObj *parser.DevfileObj) (string, error) {
	...
	if devfileObj != nil {
		...
		if name == "" || strings.TrimSpace(name) == "" {
			...
			alizerClient := alizer.Alizer{} // Tastra(feloy) fix with DI
...
func Log(platformClient platform.Client, componentName string, appName string, follow bool, command v1alpha2.Command) (io.ReadCloser, error) {
	...
	pod, err := platformClient.GetRunningPodFromSelector(astralabels.GetSelector(componentName, appName, astralabels.ComponentDevMode, false))
	if err != nil {
...
// that are both astra and non-astra components.
//
...
func ListAllClusterComponents(client kclient.ClientInterface, namespace string) ([]api.ComponentAbstract, error) {
	...
	for _, resource := range resourceList {
		...
		name := astralabels.GetComponentName(labels)
		if name == "" {
			continue
		}

		// Get the component type (if there is any..)
		componentType, err := astralabels.GetProjectType(nil, annotations)
		...
		managedBy := astralabels.GetManagedBy(labels)
		if managedBy == "" && name == "" {
			continue
		}

		managedByVersion := astralabels.GetManagedByVersion(labels)
		...
		mode := astralabels.GetMode(labels)
...
func getResourcesForComponent(
	...
	selector := astralabels.GetNameSelector(name)
...
// the resources deployed with matching labels, based on the "astra.dev/mode" label
func GetRunningModes(ctx context.Context, kubeClient kclient.ClientInterface, podmanClient podman.Client, name string) (map[platform.Client]api.RunningModes, error) {
	...
	for plt, list := range listByPlatform {
		...
		for _, resource := range list {
			...
			mode := astralabels.GetMode(resourceLabels)
			if mode != "" {
...
func checkLabelsForDevfileInfo(l1 []unstructured.Unstructured, l2 []unstructured.Unstructured) error {
	for _, k := range l1 {
		...
		var (
			...
			kName        = astralabels.GetComponentName(kLabels)
		)
		kProjectType, err := astralabels.GetProjectType(kLabels, kAnnotations)
		if err != nil {
		...
		for _, p := range l2 {
			...
			pName = astralabels.GetComponentName(pLabels)
			pProjectType, err = astralabels.GetProjectType(pLabels, pAnnotations)
			if err != nil {
...
func getDevfileInfoFromList(list []unstructured.Unstructured) (parser.DevfileObj, error) {
	...
	for _, resource := range list {
		...
		typ, err := astralabels.GetProjectType(labels, annotations)
		if err != nil {
			continue
		}
...
func ListRoutesAndIngresses(client kclient.ClientInterface, componentName, appName string) (ings []api.ConnectionData, routes []api.ConnectionData, err error) {
	...
	selector := astralabels.GetNameSelector(componentName)
