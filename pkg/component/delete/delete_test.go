...
func TestDeleteComponentClient_ListClusterResourcesToDelete(t *testing.T) {
	...
	selectorForMode := func(mode string) string {
		selector := "app.kubernetes.io/instance=my-component,app.kubernetes.io/managed-by=astra,app.kubernetes.io/part-of=app"
		if mode != "" {
			selector += fmt.Sprintf(",astra.dev/mode=%s", mode)
		}
		return selector
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(astralabels.ComponentAnyMode), "my-ns").Return(nil, nil)
					return client
				},
			},
			args: args{
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					client := kclient.NewMockClientInterface(ctrl)
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(astralabels.ComponentAnyMode), "my-ns").Return(resources, nil)
					return client
				},
			},
			args: args{
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					client := kclient.NewMockClientInterface(ctrl)
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(astralabels.ComponentDevMode), "my-ns").Return([]unstructured.Unstructured{res1}, nil)
					return client
				},
			},
			args: args{
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					client := kclient.NewMockClientInterface(ctrl)
					client.EXPECT().GetAllResourcesFromSelector(selectorForMode(astralabels.ComponentDeployMode), "my-ns").Return([]unstructured.Unstructured{res2}, nil)
					return client
				},
			},
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := astracontext.WithApplication(context.Tastra(), "app")
...
func TestDeleteComponentClient_ListClusterResourcesToDeleteFromDevfile(t *testing.T) {
	...
	// innerLoopCoreDeployment is the deployment created by astra dev for the component
	innerLoopCoreDeployment := astraTestingUtil.CreateFakeDeployment(compName, true)
	...
	// outerLoopResourceUnstructured is the deployment created by astra deploy
	...
	labeledOuterloopResource.SetLabels(astralabels.GetLabels(compName, appName, "", astralabels.ComponentDeployMode, false))

	// innerLoopResourceUnstructured is the deployment that will be deployed by apply command with `astra dev`
	innerLoopResourceUnstructured := *outerLoopResourceUnstructured.DeepCopy()
	innerLoopResourceUnstructured.SetLabels(astralabels.GetLabels(compName, appName, "", astralabels.ComponentDevMode, false))
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				devfileObj: astraTestingUtil.GetTestDevfileObjFromFile("devfile-deploy.yaml"),
				appName:    appName,
		...
		},
		{
			...
			args: args{
				devfileObj: func() parser.DevfileObj {
					obj := astraTestingUtil.GetTestDevfileObjFromFile("devfile.yaml")
		...
		},
		{
			...
			args: args{
				devfileObj: func() parser.DevfileObj {
					obj := astraTestingUtil.GetTestDevfileObjFromFile("devfile-composite-apply-commands-unit-test.yaml")
					// change the metadata name to the desired one since devfile.yaml has a different name
					metadata := obj.Data.GetMetadata()
					metadata.Name = compName
					obj.Data.SetMetadata(metadata)
		...
		},
		{
			...
			args: args{
				devfileObj: astraTestingUtil.GetTestDevfileObjFromFile("devfile-deploy-with-k8s-uri.yaml"),
				appName:    appName,
				mode:       astralabels.ComponentAnyMode,
			},
			wantIsInnerLoopDeployed: false,
...
func TestDeleteComponentClient_ExecutePreStopEvents(t *testing.T) {
	...
	devfileObjWithPreStopEvents := astraTestingUtil.GetTestDevfileObjWithPreStopEvents(fs, "runtime", "echo \"Hello World!\"")
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					selector := astralabels.GetSelector(componentName, "app", astralabels.ComponentDevMode, false)
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					selector := astralabels.GetSelector(componentName, "app", astralabels.ComponentDevMode, false)
					client.EXPECT().GetRunningPodFromSelector(selector).Return(astraTestingUtil.CreateFakePod(componentName, "mypod", "runtime"), nil)
		...
		},
		{
			...
			fields: fields{
				kubeClient: func(ctrl *gomock.Controller) kclient.ClientInterface {
					...
					selector := astralabels.GetSelector(componentName, "app", astralabels.ComponentDevMode, false)
					fakePod := astraTestingUtil.CreateFakePod(componentName, "mypod", "runtime")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx = astracontext.WithApplication(ctx, appName)
