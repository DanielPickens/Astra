...
// See https://github\.com/danielpickens/astra/wiki/Dev:-Coding-Conventions#verify-interface-compliance
...
// The goroutine spawned here can be stopped either by stopping the parent process (e.g., 'astra dev'),
...
func (k *kubeExecProcessHandler) StartProcessForCommand(ctx context.Context, def CommandDefinition, podName string, containerName string, outputHandler CommandOutputHandler) error {
	...
	// This way, returning the container logs with 'astra logs' or 'kubectl logs' would work seamlessly.
...
// shared volume (more info in the AddastraMandatoryVolume function from the utils package).
func getPidFileForCommand(def CommandDefinition) string {
	parentDir := def.PidDirectory
	if parentDir == "" {
		parentDir = storage.SharedDataMountPath
	}
	return fmt.Sprintf("%s/.astra_cmd_%s.pid", strings.TrimSuffix(parentDir, "/"), def.Id)
}
