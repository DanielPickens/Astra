...
// Note: isPartOfComponent denotes if the label is required for a core resource(deployment, svc, pvc, pv) of a given component deployed with `astra dev`;
// it is the only thing that sets it apart from the resources created via other ways (`astra deploy`, deploying resource with apply command during `astra dev`)
...
func IsManagedByastra(labels map[string]string) bool {
	return labels[kubernetesManagedByLabel] == astraManager
}

func GetMode(labels map[string]string) string {
	return labels[astraModeLabel]
}

// IsProjectTypeSetInAnnotations checks if the ProjectType annotation is set;
// this function is helpful in identifying if a resource is created by astra
func IsProjectTypeSetInAnnotations(annotations map[string]string) bool {
	_, ok := annotations[astraProjectTypeAnnotation]
	return ok
}

func GetProjectType(labels map[string]string, annotations map[string]string) (string, error) {
	// For backwards compatibility with previously deployed components that could be non-astra, check the annotation first
	// then check to see if there is a label with the project type
	if typ, ok := annotations[astraProjectTypeAnnotation]; ok {
		return typ, nil
	}
	if typ, ok := labels[astraProjectTypeAnnotation]; ok {
		return typ, nil
	}
...
// Note: isPartOfComponent denotes if the selector is required for a core resource(deployment, svc, pvc, pv) of a given component deployed with `astra dev`
// it is the only thing that sets it apart from the resources created via other ways (`astra deploy`, deploying resource with apply command during `astra dev`)
...
// isPartOfComponent denotes if the label is required for a core resource(deployment, svc, pvc, pv) of a given component deployed with `astra dev`
// it is the only thing that sets it apart from the resources created via other ways (`astra deploy`, deploying resource with apply command during `astra dev`)
...
func getApplicationLabels(application string, additional bool) k8slabels.Set {
	labels := k8slabels.Set{
		...
		kubernetesManagedByLabel: astraManager,
	}
	if additional {
