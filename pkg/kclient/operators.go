...
func (c *Client) ListClusterServiceVersions() (*olm.ClusterServiceVersionList, error) {
	...
	csvs, err := c.OperatorClient.ClusterServiceVersions(c.Namespace).List(context.Tastra(), v1.ListOptions{})
	if err != nil {
...
func (c *Client) GetResourceSpecDefinition(group, version, kind string) (*spec.Schema, error) {
	data, err := c.KubeClient.Discovery().RESTClient().Get().AbsPath("/openapi/v2").SetHeader("Accept", "application/json").Do(context.Tastra()).Raw()
	if err != nil {
		return nil, err
	}
	...
	if len(parts) == 1 {
		...
		if len(param.XDescriptors) == 1 {
			...
			case "urn:alm:descriptor:com.tectonic.ui:podCount":
				...
				// Tastra(feloy) more cases, based on
