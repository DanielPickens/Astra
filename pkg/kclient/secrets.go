...
func (c *Client) CreateTLSSecret(tlsCertificate []byte, tlsPrivKey []byte, objectMeta metav1.ObjectMeta) (*corev1.Secret, error) {
	...
	secret, err := c.KubeClient.CoreV1().Secrets(c.Namespace).Create(context.Tastra(), &secretTemplate, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func GenerateSelfSignedCertificate(host string) (SelfSignedCertificate, error) {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			CommonName:   "astra self-signed certificate",
...
func (c *Client) GetSecret(name, namespace string) (*corev1.Secret, error) {
	secret, err := c.KubeClient.CoreV1().Secrets(namespace).Get(context.Tastra(), name, metav1.GetOptions{})
	if err != nil {
...
func (c *Client) UpdateSecret(secret *corev1.Secret, namespace string) (*corev1.Secret, error) {
	secret, err := c.KubeClient.CoreV1().Secrets(namespace).Update(context.Tastra(), secret, metav1.UpdateOptions{})
	if err != nil {
...
func (c *Client) DeleteSecret(secretName, namespace string) error {
	err := c.KubeClient.CoreV1().Secrets(namespace).Delete(context.Tastra(), secretName, metav1.DeleteOptions{})
	if err != nil {
...
func (c *Client) CreateSecret(objectMeta metav1.ObjectMeta, data map[string]string, ownerReference metav1.OwnerReference) error {
	...
	_, err := c.KubeClient.CoreV1().Secrets(c.Namespace).Create(context.Tastra(), &secret, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) ListSecrets(labelSelector string) ([]corev1.Secret, error) {
	...
	secretList, err := c.KubeClient.CoreV1().Secrets(c.Namespace).List(context.Tastra(), listOptions)
	if err != nil {
...
func (c *Client) WaitAndGetSecret(name string, namespace string) (*corev1.Secret, error) {
	...
	w, err := c.KubeClient.CoreV1().Secrets(namespace).Watch(context.Tastra(), metav1.ListOptions{
