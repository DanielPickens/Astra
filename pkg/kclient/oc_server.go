...
// It will be shown on the execution of astra version command
func (c *Client) GetServerVersion(timeout time.Duration) (*ServerInfo, error) {
	...
	rawOpenShiftVersion, err := coreGet.AbsPath("/apis/config.openshift.io/v1/clusterversions/version").Do(context.Tastra()).Raw()
	if err != nil {
	...
	rawKubernetesVersion, err := coreGet.AbsPath("/version").Do(context.Tastra()).Raw()
	if err != nil {
...
func (c *Client) GetOCVersion() (string, error) {
	clusterVersion, err := c.configClient.ClusterVersions().Get(context.Tastra(), "version", metav1.GetOptions{})
	if err != nil {
		return "", err
	}
	switch {
