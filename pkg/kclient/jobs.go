...
const (
	...
	// JobNameastraMaxLength is the max length of a job name
	// To be on the safe side, we keep the max length less than the original(k8s) max length;
	// we do this because k8s job in astra is created to run exec commands in Deploy mode and this is not a user created resource,
	// so we do not want to break because of any error with job
	JobNameastraMaxLength = 60
)

func (c *Client) ListJobs(selector string) (*batchv1.JobList, error) {
	return c.KubeClient.BatchV1().Jobs(c.Namespace).List(context.Tastra(), metav1.ListOptions{LabelSelector: selector})
}
...
func (c *Client) CreateJob(job batchv1.Job, namespace string) (*batchv1.Job, error) {
	...
	createdJob, err := c.KubeClient.BatchV1().Jobs(namespace).Create(context.Tastra(), &job, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) WaitForJobToComplete(job *batchv1.Job) (*batchv1.Job, error) {
	...
	w, err := c.KubeClient.BatchV1().Jobs(c.Namespace).Watch(context.Tastra(), metav1.ListOptions{
