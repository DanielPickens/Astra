...
func (c *Client) RunLogout(stdout io.Writer) error {
	output, err := c.userClient.Users().Get(context.Tastra(), "~", metav1.GetOptions{})
	if err != nil {
	...
	if e := client.OAuthAccessTokens().Delete(context.Tastra(), conf.BearerToken, metav1.DeleteOptions{}); e != nil {
		klog.V(1).Infof("%v", e)
	}
