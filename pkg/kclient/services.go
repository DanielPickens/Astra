...
func (c *Client) CreateService(svc corev1.Service) (*corev1.Service, error) {
	service, err := c.KubeClient.CoreV1().Services(c.Namespace).Create(context.Tastra(), &svc, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) UpdateService(svc corev1.Service) (*corev1.Service, error) {
	service, err := c.KubeClient.CoreV1().Services(c.Namespace).Update(context.Tastra(), &svc, metav1.UpdateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) ListServices(selector string) ([]corev1.Service, error) {
	serviceList, err := c.KubeClient.CoreV1().Services(c.Namespace).List(context.Tastra(), metav1.ListOptions{
...
func (c *Client) DeleteService(serviceName string) error {
	err := c.KubeClient.CoreV1().Services(c.Namespace).Delete(context.Tastra(), serviceName, metav1.DeleteOptions{})
	if err != nil {
		return err
	}
	return nil
