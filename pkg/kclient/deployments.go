...
func (c *Client) GetDeploymentByName(name string) (*appsv1.Deployment, error) {
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Get(context.Tastra(), name, metav1.GetOptions{})
	// Tastra(pvala): Figure out why Kind and APIVersion are not added to the deployment object
...
func (c *Client) GetOneDeployment(componentName, appName string, isPartOfComponent bool) (*appsv1.Deployment, error) {
	selector := astralabels.GetSelector(componentName, appName, astralabels.ComponentDevMode, isPartOfComponent)
...
func (c *Client) GetDeploymentFromSelector(selector string) ([]appsv1.Deployment, error) {
	...
	if selector != "" {
		deploymentList, err = c.KubeClient.AppsV1().Deployments(c.Namespace).List(context.Tastra(), metav1.ListOptions{
...
func (c *Client) CreateDeployment(deploy appsv1.Deployment) (*appsv1.Deployment, error) {
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Create(context.Tastra(), &deploy, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) UpdateDeployment(deploy appsv1.Deployment) (*appsv1.Deployment, error) {
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Update(context.Tastra(), &deploy, metav1.UpdateOptions{FieldManager: FieldManager})
	if err != nil {
...
// It is using force:true to make sure that if someone changed one of the values that astra manages,
// astra overrides it with the value it expects instead of failing due to conflict.
func (c *Client) ApplyDeployment(deploy appsv1.Deployment) (*appsv1.Deployment, error) {
	...
	deployment, err := c.KubeClient.AppsV1().Deployments(c.Namespace).Patch(context.Tastra(), deploy.Name, types.ApplyPatchType, data, metav1.PatchOptions{FieldManager: FieldManager, Force: Bool(true)})
	if err != nil {
		return nil, fmt.Errorf("unable to update Deployment %s: %w", deploy.Name, err)
	}
	return deployment, nil
...
// https://github.com/daniel-pickens/service-binding-operator/issues/983
