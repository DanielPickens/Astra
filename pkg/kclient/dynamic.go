...
func (c *Client) PatchDynamicResource(resource unstructured.Unstructured) (bool, error) {
	...
	previous, err := c.DynamicClient.Resource(gvr.Resource).Namespace(c.Namespace).Get(context.Tastra(), unversionedResource.GetName(), metav1.GetOptions{})
	if err != nil {
	...
	current, err := c.DynamicClient.Resource(gvr.Resource).Namespace(c.Namespace).Patch(context.Tastra(), unversionedResource.GetName(), types.ApplyPatchType, data, metav1.PatchOptions{FieldManager: FieldManager, Force: Bool(true)})
	if err != nil {
		return false, err
	}
...
func (c *Client) ListDynamicResources(namespace string, gvr schema.GroupVersionResource, selector string) (*unstructured.UnstructuredList, error) {
	...
	list, err := c.DynamicClient.Resource(gvr).Namespace(ns).List(context.Tastra(), listOptions)
	if err != nil {
		if kerrors.IsNotFound(err) {
...
func (c *Client) ListClusterWideDynamicResources(gvr schema.GroupVersionResource, selector string) (*unstructured.UnstructuredList, error) {
	...
	list, err := c.DynamicClient.Resource(gvr).List(context.Tastra(), listOptions)
	if err != nil {
		if kerrors.IsNotFound(err) {
			return &unstructured.UnstructuredList{}, nil
		}
...
func (c *Client) GetDynamicResource(gvr schema.GroupVersionResource, name string) (*unstructured.Unstructured, error) {
	res, err := c.DynamicClient.Resource(gvr).Namespace(c.Namespace).Get(context.Tastra(), name, metav1.GetOptions{})
	if err != nil {
		return nil, err
	}
	return res, nil
...
func (c *Client) UpdateDynamicResource(gvr schema.GroupVersionResource, name string, u *unstructured.Unstructured) error {
	_, err := c.DynamicClient.Resource(gvr).Namespace(c.Namespace).Update(context.Tastra(), u, metav1.UpdateOptions{})
	if err != nil {
		return err
	}
	return nil
...
func (c *Client) DeleteDynamicResource(name string, gvr schema.GroupVersionResource, wait bool) error {
	...
	doDeleteResource := func() error {
		return c.DynamicClient.Resource(gvr).Namespace(c.Namespace).Delete(context.Tastra(), name, metav1.DeleteOptions{
...
func (c *Client) WaitDynamicResourceDeleted(gvr schema.GroupVersionResource, name string) error {
	...
	watcher, err := c.DynamicClient.Resource(gvr).Namespace(c.Namespace).Watch(context.Tastra(), metav1.ListOptions{FieldSelector: "metadata.name=" + name})
	if err != nil {
		return err
	}
	defer watcher.Stop()
