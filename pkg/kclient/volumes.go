...
func (c *Client) CreatePVC(pvc corev1.PersistentVolumeClaim) (*corev1.PersistentVolumeClaim, error) {
	createdPvc, err := c.KubeClient.CoreV1().PersistentVolumeClaims(c.Namespace).Create(context.Tastra(), &pvc, metav1.CreateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) DeletePVC(pvcName string) error {
	return c.KubeClient.CoreV1().PersistentVolumeClaims(c.Namespace).Delete(context.Tastra(), pvcName, metav1.DeleteOptions{})
}
...
func (c *Client) ListPVCs(selector string) ([]corev1.PersistentVolumeClaim, error) {
	pvcList, err := c.KubeClient.CoreV1().PersistentVolumeClaims(c.Namespace).List(context.Tastra(), metav1.ListOptions{
...
func (c *Client) GetPVCFromName(pvcName string) (*corev1.PersistentVolumeClaim, error) {
	return c.KubeClient.CoreV1().PersistentVolumeClaims(c.Namespace).Get(context.Tastra(), pvcName, metav1.GetOptions{})
}
...
func (c *Client) UpdatePVCLabels(pvc *corev1.PersistentVolumeClaim, labels map[string]string) error {
	pvc.Labels = labels
	_, err := c.KubeClient.CoreV1().PersistentVolumeClaims(c.Namespace).Update(context.Tastra(), pvc, metav1.UpdateOptions{FieldManager: FieldManager})
	if err != nil {
...
func (c *Client) UpdateStorageOwnerReference(pvc *corev1.PersistentVolumeClaim, ownerReference ...metav1.OwnerReference) error {
	...
	if c.IsSSASupported() {
		...
		_, err = c.KubeClient.CoreV1().PersistentVolumeClaims(c.Namespace).Patch(context.Tastra(), updatedPVC.Name, types.ApplyPatchType, data, metav1.PatchOptions{FieldManager: FieldManager, Force: Bool(true)})
		if err != nil {
			return err
		}
	} else {
		_, err := c.KubeClient.CoreV1().PersistentVolumeClaims(c.Namespace).Update(context.Tastra(), updatedPVC, metav1.UpdateOptions{FieldManager: FieldManager})
		if err != nil {
			return err
		}
	}
