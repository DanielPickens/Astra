...
func (o *LogsClient) getLogsForMode(
	...
	doneChan := make(chan struct{}) // because populating doneChan directly would cause astra logs to exit prematurely.
	...
	getPods := func() error {
		...
		if mode == astralabels.ComponentDeployMode || mode == astralabels.ComponentAnyMode {
			selector = astralabels.GetSelector(componentName, appname, astralabels.ComponentDeployMode, false)
			err := o.getPodsForSelector(selector, namespace, podChan)
			if err != nil {
				return err
			}
