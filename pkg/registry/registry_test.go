...
func TestGetDevfileRegistries(t *testing.T) {
	...
	_, err = tempConfigFile.Write([]byte(
		`kind: Preference
apiversion: astra.openshift.io/v1alpha1
astraSettings:
  RegistryList:
...
func TestListDevfileStacks(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Tastra(rm3l) Test with both nil and non-nil kubeclient
	...
	tests := []test{
		{
			...
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://github\.com/danielpickens/astra"
			},
			wantErr: true,
		},
		{
			...
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://raw.githubusercontent.com/daniel-pickens/astra"
			},
			wantErr: true,
		},
		{
			name: "GitHub-based registry: *.github.com",
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://daniel-pickens.github.com/astra"
			},
			wantErr: true,
		},
		{
			name: "GitHub-based registry: *.raw.githubusercontent.com",
			registryServerProvider: func(t *testing.T) (*httptest.Server, string) {
				return nil, "https://daniel-pickens.raw.githubusercontent.com/astra"
			},
			wantErr: true,
		},
		{
