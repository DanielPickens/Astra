...
func (o KubernetesClient) Login(server, username, password, token, caAuth string, skipTLS bool) error {
	...
	// a correctly filtered result from `astra login`
	filteredReader, filteredWriter := io.Pipe()
	go func() {
		defer filteredWriter.Close()
		_, _ = copyAndFilter(astralog.GetStdout(), filteredReader)
	}()
	...
	a := login.LoginOptions{
		...
		RequestTimeout: 0,
		IOStreams:      genericclioptions.IOStreams{Out: filteredWriter, In: os.Stdin, ErrOut: astralog.GetStderr()},
	}
	...
	astralog.Info("Connecting to the OpenShift cluster\n")
	...
	// If a new file has been created, we output what to do next (obviously astra help). This is taken from:
...
// This is used since we utilize `oc login` with astra and require certain strings to be filtered / changed
// to their astra equivalent
func filteredInformation(s []byte) []byte {

	// List of strings to correctly filter
	s = bytes.Replace(s, []byte("oc new-project"), []byte("astra create project"), -1)
	s = bytes.Replace(s, []byte("oc project "), []byte("astra set project "), -1)
	s = bytes.Replace(s, []byte("oc projects"), []byte("astra list project"), -1)
