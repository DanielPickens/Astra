...
func (o *PodmanCli) Version(ctx context.Context) (SystemVersionReport, error) {
	// Because Version is used at the very beginning of astra, when resolving and injecting dependencies (for commands that might require the Podman client),
	// it is expected to return in a timely manner (hence this configurable timeout).
	// This is to avoid situations like the one described in https://github\.com/danielpickens/astra/issues/6575
	// (where a podman CLI that takes too long to respond affects the "astra dev" command, even if the user did not intend to use the Podman platform).
