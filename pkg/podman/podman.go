...
func NewPodmanCli(ctx context.Context) (*PodmanCli, error) {
	...
	cli := &PodmanCli{
		...
		containerRunGlobalExtraArgs: envcontext.GetEnvConfig(ctx).astraContainerBackendGlobalArgs,
		containerRunExtraArgs:       envcontext.GetEnvConfig(ctx).astraContainerRunArgs,
	}
