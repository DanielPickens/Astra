...
const TelemetryClient = "astra"

// DisableTelemetryEnv is name of environment variable, if set to true it disables astra telemetry completely
...
const (
	...
	DisableTelemetryEnv = "astra_DISABLE_TELEMETRY"
	// TrackingConsentEnv controls whether astra tracks telemetry or not.
	...
	TrackingConsentEnv = "astra_TRACKING_CONSENT"
)
...
type Client struct {
	...
	// TelemetryFilePath points to the file containing anonymousID used for tracking astra commands executed by the user
...
func SetError(err error) (errString string) {
	...
	// Sanitize exec commands: For errors when a command exec fails in cases like astra exec or astra test, we do not want to know the command that the user executed, so we simply return
...
// RunningInTerminal checks if astra was run from a terminal
...
func IsTelemetryEnabled(cfg preference.Client, envConfig config.Configuration) bool {
	...
	disableTelemetry := pointer.BoolDeref(envConfig.astraDisableTelemetry, false)
	if disableTelemetry {
		//lint:ignore SA1019 We deprecated this env var, but until it is removed, we still need to support it
...
func IsTrackingConsentEnabled(envConfig *config.Configuration) (value string, enabled bool, present bool, err error) {
	if envConfig.astraTrackingConsent == nil {
		return "", false, false, nil
	}
	trackingConsent := *envConfig.astraTrackingConsent
...
// sanitizeExec sanitizes commands from the error string that might have been executed by users while running commands like astra test or astra exec
