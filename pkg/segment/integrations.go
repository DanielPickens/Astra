...
// getTelemetryForDevfileRegistry returns a populated TelemetryData object that contains some astra telemetry (with client consent), such as the anonymous ID and
// locale in addition to the generic client name "astra"
func getTelemetryForDevfileRegistry(ctx context.Context) (registryLibrary.TelemetryData, error) {
	...
	if envConfig.astraDebugTelemetryFile != nil {
		return td, nil
	}
