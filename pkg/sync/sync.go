...
func (a SyncClient) SyncFiles(ctx context.Context, syncParameters SyncParameters) (bool, error) {
	...
	if !indexRegeneratedByWatch {
		...
		// Before running the indexer, make sure the .astra folder exists (or else the index file will not get created)
		astraFolder := filepath.Join(syncParameters.Path, ".astra")
		if _, err := os.Stat(astraFolder); os.IsNotExist(err) {
			err = os.Mkdir(astraFolder, 0750)
			if err != nil {
		...
		// apply the glob rules from the .gitignore/.astraignore file
