...
func TestGetReferencedLocalFiles(t *testing.T) {
	...
	imageComponentNastrackerfile := generator.GetImageComponent(generator.ImageComponentParams{
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				devfileObj: func(fs filesystem.Filesystem) parser.DevfileObj {
					...
					_ = dData.AddComponents([]v1alpha2.Component{imageComponentNastrackerfile})
		...
		},
		{
			...
			wantResult: []string{
				"manifest.yaml", // Tastra should be parent/manifest.yaml (see https://github.com/devfile/api/issues/904)
		...
		{
			name: "With empty parent Devfile, flattened. Tastra find a way to detect the parent",
			args: args{
				devfileObj: func(fs filesystem.Filesystem) parser.DevfileObj {
					obj, _, _ := devfile.ParseDevfileAndValidate(parser.ParserArgs{
