...
func (o *State) SetForwardedPorts(ctx context.Context, fwPorts []api.ForwardedPort) error {
	...
	// Tastra(feloy) When other data is persisted into the state file, it will be needed to read the file first
...
func (o *State) writeStateFile(path string) error {
	...
	// .astra directory is supposed to exist, don't create it
...
func (o *State) checkFirstInPlatform(ctx context.Context) error {
	...
	for _, entry := range entries {
		...
		if exists {
			...
			if process.Executable() != "astra" && process.Executable() != "astra.exe" {
				klog.V(4).Infof("process %d exists but is not astra, ignoring", content.PID)
				continue
			}
...
func (o *State) GetOrphanFiles(ctx context.Context) ([]string, error) {
	...
	for _, entry := range entries {
		...
		if exists {
			...
			if process.Executable() != "astra" && process.Executable() != "astra.exe" {
				klog.V(4).Infof("process %d exists but is not astra => orphan", content.PID)
				result = append(result, filename)
				continue
			}
			// Process exists => not orphan
			klog.V(4).Infof("process %d exists and is astra => not orphan", content.PID)
			continue
		}
		klog.V(4).Infof("process %d does not exist => orphan", content.PID)
		result = append(result, filename)
