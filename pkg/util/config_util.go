...
// GetFromFile unmarshals a struct from a astra config file
func GetFromFile(c interface{}, filename string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to unmarshal astra config file: %w", err)
	}
...
func WriteToYAMLFile(c interface{}, filename string) error {
	...
	if err != nil {
		return fmt.Errorf("unable to marshal astra config data: %w", err)
	}
...
// Tastra: support deeper struct using recursion
...
func DeleteConfiguration(info interface{}, parameter string) error {
	...
	val := imm.FieldByNameFunc(CaseInsensitive(parameter))
	if !val.IsValid() {
		return fmt.Errorf("unknown parameter :'%s' is not a parameter in astra config", parameter)
	}
