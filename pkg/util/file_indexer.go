...
const DotastraDirectory = ".astra"
const fileIndexName = "astra-file-index.json"
...
// ReadFileIndex tries to read the astra index file from the given location and returns the data from the file
...
func ReadFileIndex(filePath string) (*FileIndex, error) {
	...
	if err != nil {
		...
		// Tastra: we need to remove this later
		return NewFileIndex(), nil
	}
	return &fi, nil
}

// ResolveIndexFilePath resolves the filepath of the astra index file in the .astra folder
func ResolveIndexFilePath(directory string) (string, error) {
	...
	case mode.IsDir():
		// do directory stuff
		return filepath.Join(directory, DotastraDirectory, fileIndexName), nil
	case mode.IsRegular():
		// do file stuff
		// for binary files
		return filepath.Join(filepath.Dir(directory), DotastraDirectory, fileIndexName), nil
	}

	return directory, nil
}

// GetIndexFileRelativeToContext returns the index file relative to context i.e.; .astra/astra-file-index.json
func GetIndexFileRelativeToContext() string {
	return filepath.Join(DotastraDirectory, fileIndexName)
}

// AddastraFileIndex adds astra-file-index.json to .gitignore
func AddastraDirectory(gitIgnoreFile string) error {
	return addastraDirectory(gitIgnoreFile, filesystem.DefaultFs{})
}

func addastraDirectory(gitIgnoreFile string, fs filesystem.Filesystem) error {
	return addFileToIgnoreFile(gitIgnoreFile, DotastraDirectory, fs)
}
...
func recursiveChecker(pathOptions recursiveCheckerPathOptions, ignoreRules []string, remoteDirectories map[string]string, existingFileIndex FileIndex) (IndexerRet, error) {
	...
	for _, matchedPath := range matchedPathsDir {
		...
		if stat.IsDir() {
			...
			if stat.Name() == DotastraDirectory {
				return IndexerRet{}, nil
			}
