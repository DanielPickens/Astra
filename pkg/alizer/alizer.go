...
//
// Use:
// import "github.com/daniel-pickens/alizer/pkg/apis/recognizer"
...
func (o *Alizer) DetectPorts(path string) ([]int, error) {
	//Tastra(rm3l): Find a better way not to call recognizer.DetectComponents multiple times (in DetectFramework, DetectName and DetectPorts)
	components, err := recognizer.DetectComponents(path)
	if err != nil {
		return nil, err
	}
