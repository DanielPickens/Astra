PROJECT := github\.com/danielpickens/astra
ifdef GITCOMMIT
        GITCOMMIT := $(GITCOMMIT)
else
...
# astra_LOG_LEVEL sets the verbose log level for the make tests
export astra_LOG_LEVEL ?= 4
...
prebuild: ## Step to place go embedded files into Go sources before to build the go executable
	cp astradevapispec.yaml pkg/apiserver-impl/swagger-ui/swagger.yaml

.PHONY: bin
bin: prebuild ## build the astra binary
	go build ${BUILD_FLAGS} cmd/astra/astra.go

.PHONY: release-bin
release-bin: prebuild ## build the astra binary
	go build ${RELEASE_BUILD_FLAGS} cmd/astra/astra.go

.PHONY: install
install: prebuild
	go install ${BUILD_FLAGS} ./cmd/astra/
...
.PHONY: goget-tools
goget-tools:  ## Install binaries of the tools used by astra (current list: golangci-lint, mockgen)
...
# Generate OpenAPISpec library based on astradevapispec.yaml inside pkg/apiserver-gen; this will only generate interfaces
...
# and removes openapi.yaml to avoid any confusion regarding astradevapispec.yaml file and which file to use.
.PHONY: generate-apiserver
generate-apiserver: ## Generate OpenAPISpec library based on astradevapispec.yaml inside pkg/apiserver-gen
	podman run --rm \
    		-v ${PWD}:/local \
    		docker.io/openapitools/openapi-generator-cli:v6.6.0 \
    		generate \
    		-i /local/astradevapispec.yaml \
    		-g go-server \
...
generate-apifront: ## Generate OpenAPISpec library based on astradevapispec.yaml inside ui/src/app
	podman run --rm \
    		-v ${PWD}:/local \
    		docker.io/openapitools/openapi-generator-cli:v6.6.0 \
    		generate \
...
.PHONY: generate-api
generate-api: generate-apiserver generate-apifront ## Generate code based on astradevapispec.yaml
