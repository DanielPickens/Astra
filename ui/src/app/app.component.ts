...
import { astraapiService } from './services/astraapi.service';
...
export class AppComponent implements OnInit {
  ...
  constructor(
    ...
    private astraApi: astraapiService,
  ...
  ngOnInit() {
    ...
    const devfile = this.astraApi.getDevfile();
    devfile.subscribe({
      next: (devfile) => {
    ...
    this.astraApi.telemetry().subscribe({
  ...
  propagateChange(content: string, saveToApi: boolean, fromApi: boolean){
    ...
    result.subscribe({
      next: (value) => {
        ...
        if (saveToApi) {
          this.astraApi.saveDevfile(value.content).subscribe({
            next: () => {},
            error: (error) => {
